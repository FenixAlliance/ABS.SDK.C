...
blockchain_block_dto_list_envelope_t *blockchain_block_dto_list_envelope_create(
    ...
    blockchain_block_dto_list_envelope_local_var->http_status = http_status;
    blockchain_block_dto_list_envelope_local_var->error_code = error_code;
    blockchain_block_dto_list_envelope_local_var->validation_details = validation_details;
...
void blockchain_block_dto_list_envelope_free(blockchain_block_dto_list_envelope_t *blockchain_block_dto_list_envelope) {
    ...
    if (blockchain_block_dto_list_envelope->error_code) {
        free(blockchain_block_dto_list_envelope->error_code);
        blockchain_block_dto_list_envelope->error_code = NULL;
    }
    if (blockchain_block_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, blockchain_block_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(blockchain_block_dto_list_envelope->validation_details);
        blockchain_block_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *blockchain_block_dto_list_envelope_convertToJSON(blockchain_block_dto_list_envelope_t *blockchain_block_dto_list_envelope) {
    ...
    // blockchain_block_dto_list_envelope->http_status
    if(blockchain_block_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", blockchain_block_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // blockchain_block_dto_list_envelope->error_code
    if(blockchain_block_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", blockchain_block_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // blockchain_block_dto_list_envelope->validation_details
    if(blockchain_block_dto_list_envelope->validation_details) {
    ...
    if (blockchain_block_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, blockchain_block_dto_list_envelope->validation_details) {
...
blockchain_block_dto_list_envelope_t *blockchain_block_dto_list_envelope_parseFromJSON(cJSON *blockchain_block_dto_list_envelopeJSON){
    ...
    // define the local map for blockchain_block_dto_list_envelope->validation_details
    ...
    // blockchain_block_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(blockchain_block_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // blockchain_block_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(blockchain_block_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // blockchain_block_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(blockchain_block_dto_list_envelopeJSON, "validationDetails");
