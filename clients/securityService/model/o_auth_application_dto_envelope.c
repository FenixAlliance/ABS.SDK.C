...
o_auth_application_dto_envelope_t *o_auth_application_dto_envelope_create(
    ...
    o_auth_application_dto_envelope_local_var->http_status = http_status;
    o_auth_application_dto_envelope_local_var->error_code = error_code;
    o_auth_application_dto_envelope_local_var->validation_details = validation_details;
...
void o_auth_application_dto_envelope_free(o_auth_application_dto_envelope_t *o_auth_application_dto_envelope) {
    ...
    if (o_auth_application_dto_envelope->error_code) {
        free(o_auth_application_dto_envelope->error_code);
        o_auth_application_dto_envelope->error_code = NULL;
    }
    if (o_auth_application_dto_envelope->validation_details) {
        list_ForEach(listEntry, o_auth_application_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(o_auth_application_dto_envelope->validation_details);
        o_auth_application_dto_envelope->validation_details = NULL;
    }
...
cJSON *o_auth_application_dto_envelope_convertToJSON(o_auth_application_dto_envelope_t *o_auth_application_dto_envelope) {
    ...
    // o_auth_application_dto_envelope->http_status
    if(o_auth_application_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", o_auth_application_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // o_auth_application_dto_envelope->error_code
    if(o_auth_application_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", o_auth_application_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // o_auth_application_dto_envelope->validation_details
    if(o_auth_application_dto_envelope->validation_details) {
    ...
    if (o_auth_application_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, o_auth_application_dto_envelope->validation_details) {
...
o_auth_application_dto_envelope_t *o_auth_application_dto_envelope_parseFromJSON(cJSON *o_auth_application_dto_envelopeJSON){
    ...
    // define the local map for o_auth_application_dto_envelope->validation_details
    ...
    // o_auth_application_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(o_auth_application_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // o_auth_application_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(o_auth_application_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // o_auth_application_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(o_auth_application_dto_envelopeJSON, "validationDetails");
