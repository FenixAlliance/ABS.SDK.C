...
webhook_request_dto_list_envelope_t *webhook_request_dto_list_envelope_create(
    ...
    webhook_request_dto_list_envelope_local_var->http_status = http_status;
    webhook_request_dto_list_envelope_local_var->error_code = error_code;
    webhook_request_dto_list_envelope_local_var->validation_details = validation_details;
...
void webhook_request_dto_list_envelope_free(webhook_request_dto_list_envelope_t *webhook_request_dto_list_envelope) {
    ...
    if (webhook_request_dto_list_envelope->error_code) {
        free(webhook_request_dto_list_envelope->error_code);
        webhook_request_dto_list_envelope->error_code = NULL;
    }
    if (webhook_request_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, webhook_request_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(webhook_request_dto_list_envelope->validation_details);
        webhook_request_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *webhook_request_dto_list_envelope_convertToJSON(webhook_request_dto_list_envelope_t *webhook_request_dto_list_envelope) {
    ...
    // webhook_request_dto_list_envelope->http_status
    if(webhook_request_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", webhook_request_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // webhook_request_dto_list_envelope->error_code
    if(webhook_request_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", webhook_request_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // webhook_request_dto_list_envelope->validation_details
    if(webhook_request_dto_list_envelope->validation_details) {
    ...
    if (webhook_request_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, webhook_request_dto_list_envelope->validation_details) {
...
webhook_request_dto_list_envelope_t *webhook_request_dto_list_envelope_parseFromJSON(cJSON *webhook_request_dto_list_envelopeJSON){
    ...
    // define the local map for webhook_request_dto_list_envelope->validation_details
    ...
    // webhook_request_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(webhook_request_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // webhook_request_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(webhook_request_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // webhook_request_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(webhook_request_dto_list_envelopeJSON, "validationDetails");
