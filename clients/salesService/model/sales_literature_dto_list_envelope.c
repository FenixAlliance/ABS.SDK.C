...
sales_literature_dto_list_envelope_t *sales_literature_dto_list_envelope_create(
    ...
    sales_literature_dto_list_envelope_local_var->http_status = http_status;
    sales_literature_dto_list_envelope_local_var->error_code = error_code;
    sales_literature_dto_list_envelope_local_var->validation_details = validation_details;
...
void sales_literature_dto_list_envelope_free(sales_literature_dto_list_envelope_t *sales_literature_dto_list_envelope) {
    ...
    if (sales_literature_dto_list_envelope->error_code) {
        free(sales_literature_dto_list_envelope->error_code);
        sales_literature_dto_list_envelope->error_code = NULL;
    }
    if (sales_literature_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, sales_literature_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(sales_literature_dto_list_envelope->validation_details);
        sales_literature_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *sales_literature_dto_list_envelope_convertToJSON(sales_literature_dto_list_envelope_t *sales_literature_dto_list_envelope) {
    ...
    // sales_literature_dto_list_envelope->http_status
    if(sales_literature_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", sales_literature_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // sales_literature_dto_list_envelope->error_code
    if(sales_literature_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", sales_literature_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // sales_literature_dto_list_envelope->validation_details
    if(sales_literature_dto_list_envelope->validation_details) {
    ...
    if (sales_literature_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, sales_literature_dto_list_envelope->validation_details) {
...
sales_literature_dto_list_envelope_t *sales_literature_dto_list_envelope_parseFromJSON(cJSON *sales_literature_dto_list_envelopeJSON){
    ...
    // define the local map for sales_literature_dto_list_envelope->validation_details
    ...
    // sales_literature_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(sales_literature_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // sales_literature_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(sales_literature_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // sales_literature_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(sales_literature_dto_list_envelopeJSON, "validationDetails");
