...
point_of_sale_dto_list_envelope_t *point_of_sale_dto_list_envelope_create(
    ...
    point_of_sale_dto_list_envelope_local_var->http_status = http_status;
    point_of_sale_dto_list_envelope_local_var->error_code = error_code;
    point_of_sale_dto_list_envelope_local_var->validation_details = validation_details;
...
void point_of_sale_dto_list_envelope_free(point_of_sale_dto_list_envelope_t *point_of_sale_dto_list_envelope) {
    ...
    if (point_of_sale_dto_list_envelope->error_code) {
        free(point_of_sale_dto_list_envelope->error_code);
        point_of_sale_dto_list_envelope->error_code = NULL;
    }
    if (point_of_sale_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, point_of_sale_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(point_of_sale_dto_list_envelope->validation_details);
        point_of_sale_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *point_of_sale_dto_list_envelope_convertToJSON(point_of_sale_dto_list_envelope_t *point_of_sale_dto_list_envelope) {
    ...
    // point_of_sale_dto_list_envelope->http_status
    if(point_of_sale_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", point_of_sale_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // point_of_sale_dto_list_envelope->error_code
    if(point_of_sale_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", point_of_sale_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // point_of_sale_dto_list_envelope->validation_details
    if(point_of_sale_dto_list_envelope->validation_details) {
    ...
    if (point_of_sale_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, point_of_sale_dto_list_envelope->validation_details) {
...
point_of_sale_dto_list_envelope_t *point_of_sale_dto_list_envelope_parseFromJSON(cJSON *point_of_sale_dto_list_envelopeJSON){
    ...
    // define the local map for point_of_sale_dto_list_envelope->validation_details
    ...
    // point_of_sale_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(point_of_sale_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // point_of_sale_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(point_of_sale_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // point_of_sale_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(point_of_sale_dto_list_envelopeJSON, "validationDetails");
