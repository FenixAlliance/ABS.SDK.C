...
AiProvidersAPI_getAiProvidersAsync(apiClient_t *apiClient, char *api_version, char *x_api_version)
{
    ...
    // create the path
    long sizeOfPath = strlen("/api/v2/IntelligenceService/AiProviders")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/api/v2/IntelligenceService/AiProviders");
    ...
    cJSON *AiProvidersAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    ai_provider_dto_list_envelope_t *elementToReturn = ai_provider_dto_list_envelope_parseFromJSON(AiProvidersAPIlocalVarJSON);
    cJSON_Delete(AiProvidersAPIlocalVarJSON);
