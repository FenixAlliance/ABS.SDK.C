...
#include <stdio.h>
#include <ctype.h>
#include "CognitiveAgentConversationAttachmentsAPI.h"
...
CognitiveAgentConversationAttachmentsAPI_uploadCognitiveAgentConversationAttachmentAsync(apiClient_t *apiClient, char *tenantId, char *agentId, char *conversationId, char *api_version, char *x_api_version, binary_t* file)
{
    ...
    // create the path
    long sizeOfPath = strlen("/api/v2/IntelligenceService/CognitiveAgents/{agentId}/Conversations/{conversationId}/Attachments")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/api/v2/IntelligenceService/CognitiveAgents/{agentId}/Conversations/{conversationId}/Attachments");
    ...
    cJSON *CognitiveAgentConversationAttachmentsAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    conversation_attachment_upload_result_dto_envelope_t *elementToReturn = conversation_attachment_upload_result_dto_envelope_parseFromJSON(CognitiveAgentConversationAttachmentsAPIlocalVarJSON);
    cJSON_Delete(CognitiveAgentConversationAttachmentsAPIlocalVarJSON);
