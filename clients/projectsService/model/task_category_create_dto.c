...
#include <string.h>
#include <stdio.h>
#include "task_category_create_dto.h"
...
void task_category_create_dto_free(task_category_create_dto_t *task_category_create_dto) {
    if(NULL == task_category_create_dto){
        return ;
    }
    listEntry_t *listEntry;
    if (task_category_create_dto->id) {
        free(task_category_create_dto->id);
        task_category_create_dto->id = NULL;
    }
    if (task_category_create_dto->timestamp) {
        free(task_category_create_dto->timestamp);
        task_category_create_dto->timestamp = NULL;
    }
...
cJSON *task_category_create_dto_convertToJSON(task_category_create_dto_t *task_category_create_dto) {
    ...
    // task_category_create_dto->id
    if(task_category_create_dto->id) {
    if(cJSON_AddStringToObject(item, "id", task_category_create_dto->id) == NULL) {
    goto fail; //String
    }
    }


    // task_category_create_dto->timestamp
    if(task_category_create_dto->timestamp) {
    if(cJSON_AddStringToObject(item, "timestamp", task_category_create_dto->timestamp) == NULL) {
    goto fail; //Date-Time
    }
    }
...
task_category_create_dto_t *task_category_create_dto_parseFromJSON(cJSON *task_category_create_dtoJSON){
    ...
    task_category_create_dto_t *task_category_create_dto_local_var = NULL;

    // task_category_create_dto->id
    cJSON *id = cJSON_GetObjectItemCaseSensitive(task_category_create_dtoJSON, "id");
    if (id) { 
    ...
    // task_category_create_dto->timestamp
    cJSON *timestamp = cJSON_GetObjectItemCaseSensitive(task_category_create_dtoJSON, "timestamp");
    if (timestamp) { 
    ...
    // task_category_create_dto->title
    cJSON *title = cJSON_GetObjectItemCaseSensitive(task_category_create_dtoJSON, "title");
    if (title) { 
