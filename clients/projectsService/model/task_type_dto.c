...
task_type_dto_t *task_type_dto_create(
    ...
    task_type_dto_local_var->id = id;
    task_type_dto_local_var->timestamp = timestamp;
    task_type_dto_local_var->title = title;
...
void task_type_dto_free(task_type_dto_t *task_type_dto) {
    if(NULL == task_type_dto){
        return ;
    }
    listEntry_t *listEntry;
    if (task_type_dto->id) {
        free(task_type_dto->id);
        task_type_dto->id = NULL;
    }
    if (task_type_dto->timestamp) {
        free(task_type_dto->timestamp);
        task_type_dto->timestamp = NULL;
    }
    if (task_type_dto->title) {
        free(task_type_dto->title);
        task_type_dto->title = NULL;
    }
...
cJSON *task_type_dto_convertToJSON(task_type_dto_t *task_type_dto) {
    ...
    // task_type_dto->id
    if(task_type_dto->id) {
    if(cJSON_AddStringToObject(item, "id", task_type_dto->id) == NULL) {
    goto fail; //String
    }
    }


    // task_type_dto->timestamp
    if(task_type_dto->timestamp) {
    if(cJSON_AddStringToObject(item, "timestamp", task_type_dto->timestamp) == NULL) {
    goto fail; //Date-Time
    }
    }
    ...
    // task_type_dto->title
    if(task_type_dto->title) {
    if(cJSON_AddStringToObject(item, "title", task_type_dto->title) == NULL) {
    goto fail; //String
    }
    }
    ...
    // task_type_dto->display_in_time_tracker
    if(task_type_dto->display_in_time_tracker) {
    if(cJSON_AddBoolToObject(item, "displayInTimeTracker", task_type_dto->display_in_time_tracker) == NULL) {
    goto fail; //Bool
    }
    }


    // task_type_dto->requires_description
...
task_type_dto_t *task_type_dto_parseFromJSON(cJSON *task_type_dtoJSON){
    ...
    task_type_dto_t *task_type_dto_local_var = NULL;

    // task_type_dto->id
    cJSON *id = cJSON_GetObjectItemCaseSensitive(task_type_dtoJSON, "id");
    if (id) { 
    ...
    // task_type_dto->timestamp
    cJSON *timestamp = cJSON_GetObjectItemCaseSensitive(task_type_dtoJSON, "timestamp");
    if (timestamp) { 
    ...
    // task_type_dto->title
    cJSON *title = cJSON_GetObjectItemCaseSensitive(task_type_dtoJSON, "title");
    if (title) { 
    ...
    // task_type_dto->display_in_time_tracker
    cJSON *display_in_time_tracker = cJSON_GetObjectItemCaseSensitive(task_type_dtoJSON, "displayInTimeTracker");
    ...
    // task_type_dto->requires_description
    cJSON *requires_description = cJSON_GetObjectItemCaseSensitive(task_type_dtoJSON, "requiresDescription");
