...
project_dto_list_envelope_t *project_dto_list_envelope_create(
    ...
    project_dto_list_envelope_local_var->http_status = http_status;
    project_dto_list_envelope_local_var->error_code = error_code;
    project_dto_list_envelope_local_var->validation_details = validation_details;
...
void project_dto_list_envelope_free(project_dto_list_envelope_t *project_dto_list_envelope) {
    ...
    if (project_dto_list_envelope->error_code) {
        free(project_dto_list_envelope->error_code);
        project_dto_list_envelope->error_code = NULL;
    }
    if (project_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, project_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(project_dto_list_envelope->validation_details);
        project_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *project_dto_list_envelope_convertToJSON(project_dto_list_envelope_t *project_dto_list_envelope) {
    ...
    // project_dto_list_envelope->http_status
    if(project_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", project_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // project_dto_list_envelope->error_code
    if(project_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", project_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // project_dto_list_envelope->validation_details
    if(project_dto_list_envelope->validation_details) {
    ...
    if (project_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, project_dto_list_envelope->validation_details) {
...
project_dto_list_envelope_t *project_dto_list_envelope_parseFromJSON(cJSON *project_dto_list_envelopeJSON){
    ...
    // define the local map for project_dto_list_envelope->validation_details
    ...
    // project_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(project_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // project_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(project_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // project_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(project_dto_list_envelopeJSON, "validationDetails");
