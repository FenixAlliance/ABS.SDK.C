...
asset_repair_dto_list_envelope_t *asset_repair_dto_list_envelope_create(
    ...
    asset_repair_dto_list_envelope_local_var->http_status = http_status;
    asset_repair_dto_list_envelope_local_var->error_code = error_code;
    asset_repair_dto_list_envelope_local_var->validation_details = validation_details;
...
void asset_repair_dto_list_envelope_free(asset_repair_dto_list_envelope_t *asset_repair_dto_list_envelope) {
    ...
    if (asset_repair_dto_list_envelope->error_code) {
        free(asset_repair_dto_list_envelope->error_code);
        asset_repair_dto_list_envelope->error_code = NULL;
    }
    if (asset_repair_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, asset_repair_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(asset_repair_dto_list_envelope->validation_details);
        asset_repair_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *asset_repair_dto_list_envelope_convertToJSON(asset_repair_dto_list_envelope_t *asset_repair_dto_list_envelope) {
    ...
    // asset_repair_dto_list_envelope->http_status
    if(asset_repair_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", asset_repair_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // asset_repair_dto_list_envelope->error_code
    if(asset_repair_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", asset_repair_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // asset_repair_dto_list_envelope->validation_details
    if(asset_repair_dto_list_envelope->validation_details) {
    ...
    if (asset_repair_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, asset_repair_dto_list_envelope->validation_details) {
...
asset_repair_dto_list_envelope_t *asset_repair_dto_list_envelope_parseFromJSON(cJSON *asset_repair_dto_list_envelopeJSON){
    ...
    // define the local map for asset_repair_dto_list_envelope->validation_details
    ...
    // asset_repair_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(asset_repair_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // asset_repair_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(asset_repair_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // asset_repair_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(asset_repair_dto_list_envelopeJSON, "validationDetails");
