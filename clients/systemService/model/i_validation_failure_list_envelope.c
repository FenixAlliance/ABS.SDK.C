...
i_validation_failure_list_envelope_t *i_validation_failure_list_envelope_create(
    ...
    i_validation_failure_list_envelope_local_var->http_status = http_status;
    i_validation_failure_list_envelope_local_var->error_code = error_code;
    i_validation_failure_list_envelope_local_var->validation_details = validation_details;
...
void i_validation_failure_list_envelope_free(i_validation_failure_list_envelope_t *i_validation_failure_list_envelope) {
    ...
    if (i_validation_failure_list_envelope->error_code) {
        free(i_validation_failure_list_envelope->error_code);
        i_validation_failure_list_envelope->error_code = NULL;
    }
    if (i_validation_failure_list_envelope->validation_details) {
        list_ForEach(listEntry, i_validation_failure_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(i_validation_failure_list_envelope->validation_details);
        i_validation_failure_list_envelope->validation_details = NULL;
    }
...
cJSON *i_validation_failure_list_envelope_convertToJSON(i_validation_failure_list_envelope_t *i_validation_failure_list_envelope) {
    ...
    // i_validation_failure_list_envelope->http_status
    if(i_validation_failure_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", i_validation_failure_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // i_validation_failure_list_envelope->error_code
    if(i_validation_failure_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", i_validation_failure_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // i_validation_failure_list_envelope->validation_details
    if(i_validation_failure_list_envelope->validation_details) {
    ...
    if (i_validation_failure_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, i_validation_failure_list_envelope->validation_details) {
...
i_validation_failure_list_envelope_t *i_validation_failure_list_envelope_parseFromJSON(cJSON *i_validation_failure_list_envelopeJSON){
    ...
    // define the local map for i_validation_failure_list_envelope->validation_details
    ...
    // i_validation_failure_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(i_validation_failure_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // i_validation_failure_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(i_validation_failure_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // i_validation_failure_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(i_validation_failure_list_envelopeJSON, "validationDetails");
