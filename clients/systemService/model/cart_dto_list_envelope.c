...
cart_dto_list_envelope_t *cart_dto_list_envelope_create(
    ...
    cart_dto_list_envelope_local_var->http_status = http_status;
    cart_dto_list_envelope_local_var->error_code = error_code;
    cart_dto_list_envelope_local_var->validation_details = validation_details;
...
void cart_dto_list_envelope_free(cart_dto_list_envelope_t *cart_dto_list_envelope) {
    ...
    if (cart_dto_list_envelope->error_code) {
        free(cart_dto_list_envelope->error_code);
        cart_dto_list_envelope->error_code = NULL;
    }
    if (cart_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, cart_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(cart_dto_list_envelope->validation_details);
        cart_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *cart_dto_list_envelope_convertToJSON(cart_dto_list_envelope_t *cart_dto_list_envelope) {
    ...
    // cart_dto_list_envelope->http_status
    if(cart_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", cart_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // cart_dto_list_envelope->error_code
    if(cart_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", cart_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // cart_dto_list_envelope->validation_details
    if(cart_dto_list_envelope->validation_details) {
    ...
    if (cart_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, cart_dto_list_envelope->validation_details) {
...
cart_dto_list_envelope_t *cart_dto_list_envelope_parseFromJSON(cJSON *cart_dto_list_envelopeJSON){
    ...
    // define the local map for cart_dto_list_envelope->validation_details
    ...
    // cart_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(cart_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // cart_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(cart_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // cart_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(cart_dto_list_envelopeJSON, "validationDetails");
