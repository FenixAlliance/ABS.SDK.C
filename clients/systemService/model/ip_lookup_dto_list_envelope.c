...
ip_lookup_dto_list_envelope_t *ip_lookup_dto_list_envelope_create(
    ...
    ip_lookup_dto_list_envelope_local_var->http_status = http_status;
    ip_lookup_dto_list_envelope_local_var->error_code = error_code;
    ip_lookup_dto_list_envelope_local_var->validation_details = validation_details;
...
void ip_lookup_dto_list_envelope_free(ip_lookup_dto_list_envelope_t *ip_lookup_dto_list_envelope) {
    ...
    if (ip_lookup_dto_list_envelope->error_code) {
        free(ip_lookup_dto_list_envelope->error_code);
        ip_lookup_dto_list_envelope->error_code = NULL;
    }
    if (ip_lookup_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, ip_lookup_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(ip_lookup_dto_list_envelope->validation_details);
        ip_lookup_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *ip_lookup_dto_list_envelope_convertToJSON(ip_lookup_dto_list_envelope_t *ip_lookup_dto_list_envelope) {
    ...
    // ip_lookup_dto_list_envelope->http_status
    if(ip_lookup_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", ip_lookup_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // ip_lookup_dto_list_envelope->error_code
    if(ip_lookup_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", ip_lookup_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // ip_lookup_dto_list_envelope->validation_details
    if(ip_lookup_dto_list_envelope->validation_details) {
    ...
    if (ip_lookup_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, ip_lookup_dto_list_envelope->validation_details) {
...
ip_lookup_dto_list_envelope_t *ip_lookup_dto_list_envelope_parseFromJSON(cJSON *ip_lookup_dto_list_envelopeJSON){
    ...
    // define the local map for ip_lookup_dto_list_envelope->validation_details
    ...
    // ip_lookup_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(ip_lookup_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // ip_lookup_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(ip_lookup_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // ip_lookup_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(ip_lookup_dto_list_envelopeJSON, "validationDetails");
