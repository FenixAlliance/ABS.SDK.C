...
suite_license_dto_list_envelope_t *suite_license_dto_list_envelope_create(
    ...
    suite_license_dto_list_envelope_local_var->http_status = http_status;
    suite_license_dto_list_envelope_local_var->error_code = error_code;
    suite_license_dto_list_envelope_local_var->validation_details = validation_details;
...
void suite_license_dto_list_envelope_free(suite_license_dto_list_envelope_t *suite_license_dto_list_envelope) {
    ...
    if (suite_license_dto_list_envelope->error_code) {
        free(suite_license_dto_list_envelope->error_code);
        suite_license_dto_list_envelope->error_code = NULL;
    }
    if (suite_license_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, suite_license_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(suite_license_dto_list_envelope->validation_details);
        suite_license_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *suite_license_dto_list_envelope_convertToJSON(suite_license_dto_list_envelope_t *suite_license_dto_list_envelope) {
    ...
    // suite_license_dto_list_envelope->http_status
    if(suite_license_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", suite_license_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // suite_license_dto_list_envelope->error_code
    if(suite_license_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", suite_license_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // suite_license_dto_list_envelope->validation_details
    if(suite_license_dto_list_envelope->validation_details) {
    ...
    if (suite_license_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, suite_license_dto_list_envelope->validation_details) {
...
suite_license_dto_list_envelope_t *suite_license_dto_list_envelope_parseFromJSON(cJSON *suite_license_dto_list_envelopeJSON){
    ...
    // define the local map for suite_license_dto_list_envelope->validation_details
    ...
    // suite_license_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(suite_license_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // suite_license_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(suite_license_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // suite_license_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(suite_license_dto_list_envelopeJSON, "validationDetails");
