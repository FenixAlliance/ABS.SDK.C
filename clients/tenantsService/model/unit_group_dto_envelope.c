...
unit_group_dto_envelope_t *unit_group_dto_envelope_create(
    ...
    unit_group_dto_envelope_local_var->http_status = http_status;
    unit_group_dto_envelope_local_var->error_code = error_code;
    unit_group_dto_envelope_local_var->validation_details = validation_details;
...
void unit_group_dto_envelope_free(unit_group_dto_envelope_t *unit_group_dto_envelope) {
    ...
    if (unit_group_dto_envelope->error_code) {
        free(unit_group_dto_envelope->error_code);
        unit_group_dto_envelope->error_code = NULL;
    }
    if (unit_group_dto_envelope->validation_details) {
        list_ForEach(listEntry, unit_group_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(unit_group_dto_envelope->validation_details);
        unit_group_dto_envelope->validation_details = NULL;
    }
...
cJSON *unit_group_dto_envelope_convertToJSON(unit_group_dto_envelope_t *unit_group_dto_envelope) {
    ...
    // unit_group_dto_envelope->http_status
    if(unit_group_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", unit_group_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // unit_group_dto_envelope->error_code
    if(unit_group_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", unit_group_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // unit_group_dto_envelope->validation_details
    if(unit_group_dto_envelope->validation_details) {
    ...
    if (unit_group_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, unit_group_dto_envelope->validation_details) {
...
unit_group_dto_envelope_t *unit_group_dto_envelope_parseFromJSON(cJSON *unit_group_dto_envelopeJSON){
    ...
    // define the local map for unit_group_dto_envelope->validation_details
    ...
    // unit_group_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(unit_group_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // unit_group_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(unit_group_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // unit_group_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(unit_group_dto_envelopeJSON, "validationDetails");
