...
tenant_size_dto_t *tenant_size_dto_create(
    ...
    tenant_size_dto_t *tenant_size_dto_local_var = malloc(sizeof(tenant_size_dto_t));
    if (!tenant_size_dto_local_var) {
        return NULL;
    }
    ...
    return tenant_size_dto_local_var;
}


void tenant_size_dto_free(tenant_size_dto_t *tenant_size_dto) {
    if(NULL == tenant_size_dto){
        return ;
    }
    listEntry_t *listEntry;
    if (tenant_size_dto->id) {
        free(tenant_size_dto->id);
        tenant_size_dto->id = NULL;
    }
    ...
    if (tenant_size_dto->name) {
        ...
        tenant_size_dto->name = NULL;
    }
    free(tenant_size_dto);
}
...
cJSON *tenant_size_dto_convertToJSON(tenant_size_dto_t *tenant_size_dto) {
    ...
    // tenant_size_dto->id
    if(tenant_size_dto->id) {
    if(cJSON_AddStringToObject(item, "id", tenant_size_dto->id) == NULL) {
    goto fail; //String
    }
    }


    // tenant_size_dto->timestamp
    if(tenant_size_dto->timestamp) {
    ...
    if(tenant_size_dto->name) {
    if(cJSON_AddStringToObject(item, "name", tenant_size_dto->name) == NULL) {
    goto fail; //String
    }
    }


    // tenant_size_dto->employee_low_range_value
    if(tenant_size_dto->employee_low_range_value) {
    if(cJSON_AddNumberToObject(item, "employeeLowRangeValue", tenant_size_dto->employee_low_range_value) == NULL) {
    goto fail; //Numeric
    }
    }


    // tenant_size_dto->employee_high_range_value
    if(tenant_size_dto->employee_high_range_value) {
    if(cJSON_AddNumberToObject(item, "employeeHighRangeValue", tenant_size_dto->employee_high_range_value) == NULL) {
    goto fail; //Numeric
    }
    }
...
tenant_size_dto_t *tenant_size_dto_parseFromJSON(cJSON *tenant_size_dtoJSON){
    ...
    tenant_size_dto_t *tenant_size_dto_local_var = NULL;

    // tenant_size_dto->id
    cJSON *id = cJSON_GetObjectItemCaseSensitive(tenant_size_dtoJSON, "id");
    if (id) { 
    ...
    // tenant_size_dto->timestamp
    cJSON *timestamp = cJSON_GetObjectItemCaseSensitive(tenant_size_dtoJSON, "timestamp");
    if (timestamp) { 
    ...
    // tenant_size_dto->employee_low_range_value
    cJSON *employee_low_range_value = cJSON_GetObjectItemCaseSensitive(tenant_size_dtoJSON, "employeeLowRangeValue");
    ...
    // tenant_size_dto->employee_high_range_value
    cJSON *employee_high_range_value = cJSON_GetObjectItemCaseSensitive(tenant_size_dtoJSON, "employeeHighRangeValue");
