...
unit_dto_list_envelope_t *unit_dto_list_envelope_create(
    ...
    unit_dto_list_envelope_local_var->http_status = http_status;
    unit_dto_list_envelope_local_var->error_code = error_code;
    unit_dto_list_envelope_local_var->validation_details = validation_details;
...
void unit_dto_list_envelope_free(unit_dto_list_envelope_t *unit_dto_list_envelope) {
    ...
    if (unit_dto_list_envelope->error_code) {
        free(unit_dto_list_envelope->error_code);
        unit_dto_list_envelope->error_code = NULL;
    }
    if (unit_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, unit_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(unit_dto_list_envelope->validation_details);
        unit_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *unit_dto_list_envelope_convertToJSON(unit_dto_list_envelope_t *unit_dto_list_envelope) {
    ...
    // unit_dto_list_envelope->http_status
    if(unit_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", unit_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // unit_dto_list_envelope->error_code
    if(unit_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", unit_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // unit_dto_list_envelope->validation_details
    if(unit_dto_list_envelope->validation_details) {
    ...
    if (unit_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, unit_dto_list_envelope->validation_details) {
...
unit_dto_list_envelope_t *unit_dto_list_envelope_parseFromJSON(cJSON *unit_dto_list_envelopeJSON){
    ...
    // define the local map for unit_dto_list_envelope->validation_details
    ...
    // unit_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(unit_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // unit_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(unit_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // unit_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(unit_dto_list_envelopeJSON, "validationDetails");
