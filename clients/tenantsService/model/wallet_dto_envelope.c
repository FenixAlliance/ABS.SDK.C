...
wallet_dto_envelope_t *wallet_dto_envelope_create(
    ...
    wallet_dto_envelope_local_var->http_status = http_status;
    wallet_dto_envelope_local_var->error_code = error_code;
    wallet_dto_envelope_local_var->validation_details = validation_details;
...
void wallet_dto_envelope_free(wallet_dto_envelope_t *wallet_dto_envelope) {
    ...
    if (wallet_dto_envelope->error_code) {
        free(wallet_dto_envelope->error_code);
        wallet_dto_envelope->error_code = NULL;
    }
    if (wallet_dto_envelope->validation_details) {
        list_ForEach(listEntry, wallet_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(wallet_dto_envelope->validation_details);
        wallet_dto_envelope->validation_details = NULL;
    }
...
cJSON *wallet_dto_envelope_convertToJSON(wallet_dto_envelope_t *wallet_dto_envelope) {
    ...
    // wallet_dto_envelope->http_status
    if(wallet_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", wallet_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // wallet_dto_envelope->error_code
    if(wallet_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", wallet_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // wallet_dto_envelope->validation_details
    if(wallet_dto_envelope->validation_details) {
    ...
    if (wallet_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, wallet_dto_envelope->validation_details) {
...
wallet_dto_envelope_t *wallet_dto_envelope_parseFromJSON(cJSON *wallet_dto_envelopeJSON){
    ...
    // define the local map for wallet_dto_envelope->validation_details
    ...
    // wallet_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(wallet_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // wallet_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(wallet_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // wallet_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(wallet_dto_envelopeJSON, "validationDetails");
