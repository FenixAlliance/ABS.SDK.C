...
string_list_envelope_t *string_list_envelope_create(
    ...
    string_list_envelope_local_var->http_status = http_status;
    string_list_envelope_local_var->error_code = error_code;
    string_list_envelope_local_var->validation_details = validation_details;
...
void string_list_envelope_free(string_list_envelope_t *string_list_envelope) {
    ...
    if (string_list_envelope->error_code) {
        free(string_list_envelope->error_code);
        string_list_envelope->error_code = NULL;
    }
    if (string_list_envelope->validation_details) {
        list_ForEach(listEntry, string_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(string_list_envelope->validation_details);
        string_list_envelope->validation_details = NULL;
    }
...
cJSON *string_list_envelope_convertToJSON(string_list_envelope_t *string_list_envelope) {
    ...
    // string_list_envelope->http_status
    if(string_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", string_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // string_list_envelope->error_code
    if(string_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", string_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // string_list_envelope->validation_details
    if(string_list_envelope->validation_details) {
    ...
    if (string_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, string_list_envelope->validation_details) {
...
string_list_envelope_t *string_list_envelope_parseFromJSON(cJSON *string_list_envelopeJSON){
    ...
    // define the local map for string_list_envelope->validation_details
    ...
    // string_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(string_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // string_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(string_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // string_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(string_list_envelopeJSON, "validationDetails");
