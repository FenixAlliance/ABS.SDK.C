...
tenant_segment_dto_list_envelope_t *tenant_segment_dto_list_envelope_create(
    ...
    tenant_segment_dto_list_envelope_local_var->http_status = http_status;
    tenant_segment_dto_list_envelope_local_var->error_code = error_code;
    tenant_segment_dto_list_envelope_local_var->validation_details = validation_details;
...
void tenant_segment_dto_list_envelope_free(tenant_segment_dto_list_envelope_t *tenant_segment_dto_list_envelope) {
    ...
    if (tenant_segment_dto_list_envelope->error_code) {
        free(tenant_segment_dto_list_envelope->error_code);
        tenant_segment_dto_list_envelope->error_code = NULL;
    }
    if (tenant_segment_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, tenant_segment_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(tenant_segment_dto_list_envelope->validation_details);
        tenant_segment_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *tenant_segment_dto_list_envelope_convertToJSON(tenant_segment_dto_list_envelope_t *tenant_segment_dto_list_envelope) {
    ...
    // tenant_segment_dto_list_envelope->http_status
    if(tenant_segment_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", tenant_segment_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // tenant_segment_dto_list_envelope->error_code
    if(tenant_segment_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", tenant_segment_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // tenant_segment_dto_list_envelope->validation_details
    if(tenant_segment_dto_list_envelope->validation_details) {
    ...
    if (tenant_segment_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, tenant_segment_dto_list_envelope->validation_details) {
...
tenant_segment_dto_list_envelope_t *tenant_segment_dto_list_envelope_parseFromJSON(cJSON *tenant_segment_dto_list_envelopeJSON){
    ...
    // define the local map for tenant_segment_dto_list_envelope->validation_details
    ...
    // tenant_segment_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(tenant_segment_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // tenant_segment_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(tenant_segment_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // tenant_segment_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(tenant_segment_dto_list_envelopeJSON, "validationDetails");
