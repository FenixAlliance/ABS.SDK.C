...
currency_dto_list_envelope_t *currency_dto_list_envelope_create(
    ...
    currency_dto_list_envelope_local_var->http_status = http_status;
    currency_dto_list_envelope_local_var->error_code = error_code;
    currency_dto_list_envelope_local_var->validation_details = validation_details;
...
void currency_dto_list_envelope_free(currency_dto_list_envelope_t *currency_dto_list_envelope) {
    ...
    if (currency_dto_list_envelope->error_code) {
        free(currency_dto_list_envelope->error_code);
        currency_dto_list_envelope->error_code = NULL;
    }
    if (currency_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, currency_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(currency_dto_list_envelope->validation_details);
        currency_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *currency_dto_list_envelope_convertToJSON(currency_dto_list_envelope_t *currency_dto_list_envelope) {
    ...
    // currency_dto_list_envelope->http_status
    if(currency_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", currency_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // currency_dto_list_envelope->error_code
    if(currency_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", currency_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // currency_dto_list_envelope->validation_details
    if(currency_dto_list_envelope->validation_details) {
    ...
    if (currency_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, currency_dto_list_envelope->validation_details) {
...
currency_dto_list_envelope_t *currency_dto_list_envelope_parseFromJSON(cJSON *currency_dto_list_envelopeJSON){
    ...
    // define the local map for currency_dto_list_envelope->validation_details
    ...
    // currency_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(currency_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // currency_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(currency_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // currency_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(currency_dto_list_envelopeJSON, "validationDetails");
