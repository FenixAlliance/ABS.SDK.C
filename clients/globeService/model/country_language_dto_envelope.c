...
country_language_dto_envelope_t *country_language_dto_envelope_create(
    ...
    country_language_dto_envelope_local_var->http_status = http_status;
    country_language_dto_envelope_local_var->error_code = error_code;
    country_language_dto_envelope_local_var->validation_details = validation_details;
...
void country_language_dto_envelope_free(country_language_dto_envelope_t *country_language_dto_envelope) {
    ...
    if (country_language_dto_envelope->error_code) {
        free(country_language_dto_envelope->error_code);
        country_language_dto_envelope->error_code = NULL;
    }
    if (country_language_dto_envelope->validation_details) {
        list_ForEach(listEntry, country_language_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(country_language_dto_envelope->validation_details);
        country_language_dto_envelope->validation_details = NULL;
    }
...
cJSON *country_language_dto_envelope_convertToJSON(country_language_dto_envelope_t *country_language_dto_envelope) {
    ...
    // country_language_dto_envelope->http_status
    if(country_language_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", country_language_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // country_language_dto_envelope->error_code
    if(country_language_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", country_language_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // country_language_dto_envelope->validation_details
    if(country_language_dto_envelope->validation_details) {
    ...
    if (country_language_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, country_language_dto_envelope->validation_details) {
...
country_language_dto_envelope_t *country_language_dto_envelope_parseFromJSON(cJSON *country_language_dto_envelopeJSON){
    ...
    // define the local map for country_language_dto_envelope->validation_details
    ...
    // country_language_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(country_language_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // country_language_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(country_language_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // country_language_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(country_language_dto_envelopeJSON, "validationDetails");
