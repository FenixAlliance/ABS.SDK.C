...
city_dto_list_envelope_t *city_dto_list_envelope_create(
    ...
    city_dto_list_envelope_local_var->http_status = http_status;
    city_dto_list_envelope_local_var->error_code = error_code;
    city_dto_list_envelope_local_var->validation_details = validation_details;
...
void city_dto_list_envelope_free(city_dto_list_envelope_t *city_dto_list_envelope) {
    ...
    if (city_dto_list_envelope->error_code) {
        free(city_dto_list_envelope->error_code);
        city_dto_list_envelope->error_code = NULL;
    }
    if (city_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, city_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(city_dto_list_envelope->validation_details);
        city_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *city_dto_list_envelope_convertToJSON(city_dto_list_envelope_t *city_dto_list_envelope) {
    ...
    // city_dto_list_envelope->http_status
    if(city_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", city_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // city_dto_list_envelope->error_code
    if(city_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", city_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // city_dto_list_envelope->validation_details
    if(city_dto_list_envelope->validation_details) {
    ...
    if (city_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, city_dto_list_envelope->validation_details) {
...
city_dto_list_envelope_t *city_dto_list_envelope_parseFromJSON(cJSON *city_dto_list_envelopeJSON){
    ...
    // define the local map for city_dto_list_envelope->validation_details
    ...
    // city_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(city_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // city_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(city_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // city_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(city_dto_list_envelopeJSON, "validationDetails");
