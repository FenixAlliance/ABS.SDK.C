...
country_dto_list_envelope_t *country_dto_list_envelope_create(
    ...
    country_dto_list_envelope_local_var->http_status = http_status;
    country_dto_list_envelope_local_var->error_code = error_code;
    country_dto_list_envelope_local_var->validation_details = validation_details;
...
void country_dto_list_envelope_free(country_dto_list_envelope_t *country_dto_list_envelope) {
    ...
    if (country_dto_list_envelope->error_code) {
        free(country_dto_list_envelope->error_code);
        country_dto_list_envelope->error_code = NULL;
    }
    if (country_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, country_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(country_dto_list_envelope->validation_details);
        country_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *country_dto_list_envelope_convertToJSON(country_dto_list_envelope_t *country_dto_list_envelope) {
    ...
    // country_dto_list_envelope->http_status
    if(country_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", country_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // country_dto_list_envelope->error_code
    if(country_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", country_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // country_dto_list_envelope->validation_details
    if(country_dto_list_envelope->validation_details) {
    ...
    if (country_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, country_dto_list_envelope->validation_details) {
...
country_dto_list_envelope_t *country_dto_list_envelope_parseFromJSON(cJSON *country_dto_list_envelopeJSON){
    ...
    // define the local map for country_dto_list_envelope->validation_details
    ...
    // country_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(country_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // country_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(country_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // country_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(country_dto_list_envelopeJSON, "validationDetails");
