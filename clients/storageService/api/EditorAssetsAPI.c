...
EditorAssetsAPI_getEditorAssetAsync(apiClient_t *apiClient, char *fileId, char *api_version, char *x_api_version)
{
    ...
    // create the path
    long sizeOfPath = strlen("/api/v2/StorageService/EditorAssets/{fileId}")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/api/v2/StorageService/EditorAssets/{fileId}");
