...
blob_envelope_t *blob_envelope_create(
    ...
    blob_envelope_local_var->http_status = http_status;
    blob_envelope_local_var->error_code = error_code;
    blob_envelope_local_var->validation_details = validation_details;
...
void blob_envelope_free(blob_envelope_t *blob_envelope) {
    ...
    if (blob_envelope->error_code) {
        free(blob_envelope->error_code);
        blob_envelope->error_code = NULL;
    }
    if (blob_envelope->validation_details) {
        list_ForEach(listEntry, blob_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(blob_envelope->validation_details);
        blob_envelope->validation_details = NULL;
    }
...
cJSON *blob_envelope_convertToJSON(blob_envelope_t *blob_envelope) {
    ...
    // blob_envelope->http_status
    if(blob_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", blob_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // blob_envelope->error_code
    if(blob_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", blob_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // blob_envelope->validation_details
    if(blob_envelope->validation_details) {
    ...
    if (blob_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, blob_envelope->validation_details) {
...
blob_envelope_t *blob_envelope_parseFromJSON(cJSON *blob_envelopeJSON){
    ...
    // define the local map for blob_envelope->validation_details
    ...
    // blob_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(blob_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // blob_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(blob_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // blob_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(blob_envelopeJSON, "validationDetails");
