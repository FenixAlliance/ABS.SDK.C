...
catalog_item_dto_list_envelope_t *catalog_item_dto_list_envelope_create(
    ...
    catalog_item_dto_list_envelope_local_var->http_status = http_status;
    catalog_item_dto_list_envelope_local_var->error_code = error_code;
    catalog_item_dto_list_envelope_local_var->validation_details = validation_details;
...
void catalog_item_dto_list_envelope_free(catalog_item_dto_list_envelope_t *catalog_item_dto_list_envelope) {
    ...
    if (catalog_item_dto_list_envelope->error_code) {
        free(catalog_item_dto_list_envelope->error_code);
        catalog_item_dto_list_envelope->error_code = NULL;
    }
    if (catalog_item_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, catalog_item_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(catalog_item_dto_list_envelope->validation_details);
        catalog_item_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *catalog_item_dto_list_envelope_convertToJSON(catalog_item_dto_list_envelope_t *catalog_item_dto_list_envelope) {
    ...
    // catalog_item_dto_list_envelope->http_status
    if(catalog_item_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", catalog_item_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // catalog_item_dto_list_envelope->error_code
    if(catalog_item_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", catalog_item_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // catalog_item_dto_list_envelope->validation_details
    if(catalog_item_dto_list_envelope->validation_details) {
    ...
    if (catalog_item_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, catalog_item_dto_list_envelope->validation_details) {
...
catalog_item_dto_list_envelope_t *catalog_item_dto_list_envelope_parseFromJSON(cJSON *catalog_item_dto_list_envelopeJSON){
    ...
    // define the local map for catalog_item_dto_list_envelope->validation_details
    ...
    // catalog_item_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(catalog_item_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // catalog_item_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(catalog_item_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // catalog_item_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(catalog_item_dto_list_envelopeJSON, "validationDetails");
