...
#include <string.h>
#include <stdio.h>
#include "item_family_update_dto.h"
...
item_family_update_dto_t *item_family_update_dto_create(
    ...
    if (!item_family_update_dto_local_var) {
        return NULL;
    }
...
void item_family_update_dto_free(item_family_update_dto_t *item_family_update_dto) {
    if(NULL == item_family_update_dto){
        return ;
    }
...
cJSON *item_family_update_dto_convertToJSON(item_family_update_dto_t *item_family_update_dto) {
    ...
    if(item_family_update_dto->description) {
    if(cJSON_AddStringToObject(item, "description", item_family_update_dto->description) == NULL) {
    goto fail; //String
    }
    }
...
item_family_update_dto_t *item_family_update_dto_parseFromJSON(cJSON *item_family_update_dtoJSON){
    ...
    item_family_update_dto_t *item_family_update_dto_local_var = NULL;

    // item_family_update_dto->name
    cJSON *name = cJSON_GetObjectItemCaseSensitive(item_family_update_dtoJSON, "name");
    if (!name) {
        goto end;
    }
