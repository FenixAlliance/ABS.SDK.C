...
item_attachment_dto_t *item_attachment_dto_create(
    ...
    item_attachment_dto_local_var->storage_key = storage_key;
    item_attachment_dto_local_var->storage_provider_key = storage_provider_key;
    ...
    item_attachment_dto_local_var->scan_status = scan_status;
    item_attachment_dto_local_var->thumbnail_status = thumbnail_status;
    item_attachment_dto_local_var->has_thumbnail = has_thumbnail;
    item_attachment_dto_local_var->thumbnail_storage_key = thumbnail_storage_key;
    item_attachment_dto_local_var->thumbnail_content_type = thumbnail_content_type;
    item_attachment_dto_local_var->thumbnail_width = thumbnail_width;
    item_attachment_dto_local_var->thumbnail_height = thumbnail_height;
    item_attachment_dto_local_var->public_access_type = public_access_type;
...
void item_attachment_dto_free(item_attachment_dto_t *item_attachment_dto) {
    ...
    if (item_attachment_dto->storage_key) {
        free(item_attachment_dto->storage_key);
        item_attachment_dto->storage_key = NULL;
    }
    if (item_attachment_dto->storage_provider_key) {
        free(item_attachment_dto->storage_provider_key);
        item_attachment_dto->storage_provider_key = NULL;
    }
    ...
    if (item_attachment_dto->thumbnail_storage_key) {
        free(item_attachment_dto->thumbnail_storage_key);
        item_attachment_dto->thumbnail_storage_key = NULL;
    }
    if (item_attachment_dto->thumbnail_content_type) {
        free(item_attachment_dto->thumbnail_content_type);
        item_attachment_dto->thumbnail_content_type = NULL;
    }
...
cJSON *item_attachment_dto_convertToJSON(item_attachment_dto_t *item_attachment_dto) {
    ...
    // item_attachment_dto->storage_key
    if(item_attachment_dto->storage_key) {
    if(cJSON_AddStringToObject(item, "storageKey", item_attachment_dto->storage_key) == NULL) {
    goto fail; //String
    }
    }


    // item_attachment_dto->storage_provider_key
    if(item_attachment_dto->storage_provider_key) {
    if(cJSON_AddStringToObject(item, "storageProviderKey", item_attachment_dto->storage_provider_key) == NULL) {
    goto fail; //String
    }
    }
    ...
    // item_attachment_dto->scan_status
    if(item_attachment_dto->scan_status != catalogservice_item_attachment_dto_SCANSTATUS_NULL) {
    if(cJSON_AddStringToObject(item, "scanStatus", scan_statusitem_attachment_dto_ToString(item_attachment_dto->scan_status)) == NULL)
    {
    goto fail; //Enum
    }
    }
    ...
    // item_attachment_dto->thumbnail_status
    if(item_attachment_dto->thumbnail_status != catalogservice_item_attachment_dto_THUMBNAILSTATUS_NULL) {
    if(cJSON_AddStringToObject(item, "thumbnailStatus", thumbnail_statusitem_attachment_dto_ToString(item_attachment_dto->thumbnail_status)) == NULL)
    {
    goto fail; //Enum
    }
    }
    ...
    // item_attachment_dto->has_thumbnail
    if(item_attachment_dto->has_thumbnail) {
    if(cJSON_AddBoolToObject(item, "hasThumbnail", item_attachment_dto->has_thumbnail) == NULL) {
    goto fail; //Bool
    }
    }


    // item_attachment_dto->thumbnail_storage_key
    if(item_attachment_dto->thumbnail_storage_key) {
    if(cJSON_AddStringToObject(item, "thumbnailStorageKey", item_attachment_dto->thumbnail_storage_key) == NULL) {
    goto fail; //String
    }
    }


    // item_attachment_dto->thumbnail_content_type
    if(item_attachment_dto->thumbnail_content_type) {
    if(cJSON_AddStringToObject(item, "thumbnailContentType", item_attachment_dto->thumbnail_content_type) == NULL) {
    goto fail; //String
    }
    }


    // item_attachment_dto->thumbnail_width
    if(item_attachment_dto->thumbnail_width) {
    if(cJSON_AddNumberToObject(item, "thumbnailWidth", item_attachment_dto->thumbnail_width) == NULL) {
    goto fail; //Numeric
    }
    }


    // item_attachment_dto->thumbnail_height
    if(item_attachment_dto->thumbnail_height) {
    if(cJSON_AddNumberToObject(item, "thumbnailHeight", item_attachment_dto->thumbnail_height) == NULL) {
    goto fail; //Numeric
    }
    }


    // item_attachment_dto->public_access_type
    if(item_attachment_dto->public_access_type != catalogservice_item_attachment_dto_PUBLICACCESSTYPE_NULL) {
    if(cJSON_AddStringToObject(item, "publicAccessType", public_access_typeitem_attachment_dto_ToString(item_attachment_dto->public_access_type)) == NULL)
    {
    goto fail; //Enum
    }
    }
...
item_attachment_dto_t *item_attachment_dto_parseFromJSON(cJSON *item_attachment_dtoJSON){
    ...
    // item_attachment_dto->storage_key
    cJSON *storage_key = cJSON_GetObjectItemCaseSensitive(item_attachment_dtoJSON, "storageKey");
    if (storage_key) { 
    ...
    // item_attachment_dto->storage_provider_key
    cJSON *storage_provider_key = cJSON_GetObjectItemCaseSensitive(item_attachment_dtoJSON, "storageProviderKey");
    ...
    // item_attachment_dto->scan_status
    cJSON *scan_status = cJSON_GetObjectItemCaseSensitive(item_attachment_dtoJSON, "scanStatus");
    catalogservice_item_attachment_dto_SCANSTATUS_e scan_statusVariable;
    if (scan_status) { 
    if(!cJSON_IsString(scan_status))
    {
    goto end; //Enum
    }
    scan_statusVariable = item_attachment_dto_scan_status_FromString(scan_status->valuestring);
    }

    // item_attachment_dto->thumbnail_status
    cJSON *thumbnail_status = cJSON_GetObjectItemCaseSensitive(item_attachment_dtoJSON, "thumbnailStatus");
    catalogservice_item_attachment_dto_THUMBNAILSTATUS_e thumbnail_statusVariable;
    if (thumbnail_status) { 
    if(!cJSON_IsString(thumbnail_status))
    {
    goto end; //Enum
    }
    thumbnail_statusVariable = item_attachment_dto_thumbnail_status_FromString(thumbnail_status->valuestring);
    }

    // item_attachment_dto->has_thumbnail
    cJSON *has_thumbnail = cJSON_GetObjectItemCaseSensitive(item_attachment_dtoJSON, "hasThumbnail");
    if (has_thumbnail) { 
    ...
    // item_attachment_dto->thumbnail_storage_key
    cJSON *thumbnail_storage_key = cJSON_GetObjectItemCaseSensitive(item_attachment_dtoJSON, "thumbnailStorageKey");
    ...
    // item_attachment_dto->thumbnail_content_type
    cJSON *thumbnail_content_type = cJSON_GetObjectItemCaseSensitive(item_attachment_dtoJSON, "thumbnailContentType");
    ...
    // item_attachment_dto->thumbnail_width
    cJSON *thumbnail_width = cJSON_GetObjectItemCaseSensitive(item_attachment_dtoJSON, "thumbnailWidth");
    if (thumbnail_width) { 
    ...
    // item_attachment_dto->thumbnail_height
    cJSON *thumbnail_height = cJSON_GetObjectItemCaseSensitive(item_attachment_dtoJSON, "thumbnailHeight");
    if (thumbnail_height) { 
    ...
    // item_attachment_dto->public_access_type
    cJSON *public_access_type = cJSON_GetObjectItemCaseSensitive(item_attachment_dtoJSON, "publicAccessType");
    catalogservice_item_attachment_dto_PUBLICACCESSTYPE_e public_access_typeVariable;
    if (public_access_type) { 
    if(!cJSON_IsString(public_access_type))
    {
    goto end; //Enum
    }
    public_access_typeVariable = item_attachment_dto_public_access_type_FromString(public_access_type->valuestring);
    }
    ...
    item_attachment_dto_local_var = item_attachment_dto_create (
        ...
        scan_status ? scan_statusVariable : catalogservice_item_attachment_dto_SCANSTATUS_NULL,
        thumbnail_status ? thumbnail_statusVariable : catalogservice_item_attachment_dto_THUMBNAILSTATUS_NULL,
        ...
        public_access_type ? public_access_typeVariable : catalogservice_item_attachment_dto_PUBLICACCESSTYPE_NULL,
