...
item_brand_dto_envelope_t *item_brand_dto_envelope_create(
    ...
    item_brand_dto_envelope_local_var->http_status = http_status;
    item_brand_dto_envelope_local_var->error_code = error_code;
    item_brand_dto_envelope_local_var->validation_details = validation_details;
...
void item_brand_dto_envelope_free(item_brand_dto_envelope_t *item_brand_dto_envelope) {
    ...
    if (item_brand_dto_envelope->error_code) {
        free(item_brand_dto_envelope->error_code);
        item_brand_dto_envelope->error_code = NULL;
    }
    if (item_brand_dto_envelope->validation_details) {
        list_ForEach(listEntry, item_brand_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(item_brand_dto_envelope->validation_details);
        item_brand_dto_envelope->validation_details = NULL;
    }
...
cJSON *item_brand_dto_envelope_convertToJSON(item_brand_dto_envelope_t *item_brand_dto_envelope) {
    ...
    // item_brand_dto_envelope->http_status
    if(item_brand_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", item_brand_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // item_brand_dto_envelope->error_code
    if(item_brand_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", item_brand_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // item_brand_dto_envelope->validation_details
    if(item_brand_dto_envelope->validation_details) {
    ...
    if (item_brand_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, item_brand_dto_envelope->validation_details) {
...
item_brand_dto_envelope_t *item_brand_dto_envelope_parseFromJSON(cJSON *item_brand_dto_envelopeJSON){
    ...
    // define the local map for item_brand_dto_envelope->validation_details
    ...
    // item_brand_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(item_brand_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // item_brand_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(item_brand_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // item_brand_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(item_brand_dto_envelopeJSON, "validationDetails");
