...
merchant_dto_envelope_t *merchant_dto_envelope_create(
    ...
    merchant_dto_envelope_local_var->http_status = http_status;
    merchant_dto_envelope_local_var->error_code = error_code;
    merchant_dto_envelope_local_var->validation_details = validation_details;
...
void merchant_dto_envelope_free(merchant_dto_envelope_t *merchant_dto_envelope) {
    ...
    if (merchant_dto_envelope->error_code) {
        free(merchant_dto_envelope->error_code);
        merchant_dto_envelope->error_code = NULL;
    }
    if (merchant_dto_envelope->validation_details) {
        list_ForEach(listEntry, merchant_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(merchant_dto_envelope->validation_details);
        merchant_dto_envelope->validation_details = NULL;
    }
...
cJSON *merchant_dto_envelope_convertToJSON(merchant_dto_envelope_t *merchant_dto_envelope) {
    ...
    // merchant_dto_envelope->http_status
    if(merchant_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", merchant_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // merchant_dto_envelope->error_code
    if(merchant_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", merchant_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // merchant_dto_envelope->validation_details
    if(merchant_dto_envelope->validation_details) {
    ...
    if (merchant_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, merchant_dto_envelope->validation_details) {
...
merchant_dto_envelope_t *merchant_dto_envelope_parseFromJSON(cJSON *merchant_dto_envelopeJSON){
    ...
    // define the local map for merchant_dto_envelope->validation_details
    ...
    // merchant_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(merchant_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // merchant_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(merchant_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // merchant_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(merchant_dto_envelopeJSON, "validationDetails");
