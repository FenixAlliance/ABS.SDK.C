...
item_tag_dto_t *item_tag_dto_create(
    ...
    item_tag_dto_local_var->tenant_id = tenant_id;
...
void item_tag_dto_free(item_tag_dto_t *item_tag_dto) {
    ...
    if (item_tag_dto->tenant_id) {
        free(item_tag_dto->tenant_id);
        item_tag_dto->tenant_id = NULL;
    }
    free(item_tag_dto);
}
...
cJSON *item_tag_dto_convertToJSON(item_tag_dto_t *item_tag_dto) {
    ...
    // item_tag_dto->tenant_id
    if(item_tag_dto->tenant_id) {
    if(cJSON_AddStringToObject(item, "tenantId", item_tag_dto->tenant_id) == NULL) {
    goto fail; //String
    }
    }
...
item_tag_dto_t *item_tag_dto_parseFromJSON(cJSON *item_tag_dtoJSON){
    ...
    // item_tag_dto->tenant_id
    cJSON *tenant_id = cJSON_GetObjectItemCaseSensitive(item_tag_dtoJSON, "tenantId");
    if (tenant_id) { 
