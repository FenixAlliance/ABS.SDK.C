...
TrackingPixelsAPI_getTrackingPixelAsync(apiClient_t *apiClient, char *pixelId, char *api_version, char *x_api_version)
{
    ...
    // create the path
    long sizeOfPath = strlen("/api/v2/MarketingService/TrackingPixels/{pixelId}")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/api/v2/MarketingService/TrackingPixels/{pixelId}");


    // Path Params
    long sizeOfPathParams_pixelId = strlen(pixelId)+3 + strlen("{ pixelId }");
    if(pixelId == NULL) {
        goto end;
    }
    char* localVarToReplace_pixelId = malloc(sizeOfPathParams_pixelId);
    sprintf(localVarToReplace_pixelId, "{%s}", "pixelId");

    localVarPath = strReplace(localVarPath, localVarToReplace_pixelId, pixelId);
    if(pixelId == NULL) {
        goto end;
    }
    char* localVarToReplace_pixelId = malloc(sizeOfPathParams_pixelId);
    ...
    cJSON *TrackingPixelsAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    order_dto_envelope_t *elementToReturn = order_dto_envelope_parseFromJSON(TrackingPixelsAPIlocalVarJSON);
    cJSON_Delete(TrackingPixelsAPIlocalVarJSON);
    ...
    free(localVarPath);
    free(localVarToReplace_pixelId);
