...
marketing_campaign_dto_envelope_t *marketing_campaign_dto_envelope_create(
    ...
    marketing_campaign_dto_envelope_local_var->http_status = http_status;
    marketing_campaign_dto_envelope_local_var->error_code = error_code;
    marketing_campaign_dto_envelope_local_var->validation_details = validation_details;
...
void marketing_campaign_dto_envelope_free(marketing_campaign_dto_envelope_t *marketing_campaign_dto_envelope) {
    ...
    if (marketing_campaign_dto_envelope->error_code) {
        free(marketing_campaign_dto_envelope->error_code);
        marketing_campaign_dto_envelope->error_code = NULL;
    }
    if (marketing_campaign_dto_envelope->validation_details) {
        list_ForEach(listEntry, marketing_campaign_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(marketing_campaign_dto_envelope->validation_details);
        marketing_campaign_dto_envelope->validation_details = NULL;
    }
...
cJSON *marketing_campaign_dto_envelope_convertToJSON(marketing_campaign_dto_envelope_t *marketing_campaign_dto_envelope) {
    ...
    // marketing_campaign_dto_envelope->http_status
    if(marketing_campaign_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", marketing_campaign_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // marketing_campaign_dto_envelope->error_code
    if(marketing_campaign_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", marketing_campaign_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // marketing_campaign_dto_envelope->validation_details
    if(marketing_campaign_dto_envelope->validation_details) {
    ...
    if (marketing_campaign_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, marketing_campaign_dto_envelope->validation_details) {
...
marketing_campaign_dto_envelope_t *marketing_campaign_dto_envelope_parseFromJSON(cJSON *marketing_campaign_dto_envelopeJSON){
    ...
    // define the local map for marketing_campaign_dto_envelope->validation_details
    ...
    // marketing_campaign_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(marketing_campaign_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // marketing_campaign_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(marketing_campaign_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // marketing_campaign_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(marketing_campaign_dto_envelopeJSON, "validationDetails");
