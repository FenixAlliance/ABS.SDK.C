...
email_template_dto_envelope_t *email_template_dto_envelope_create(
    ...
    email_template_dto_envelope_local_var->http_status = http_status;
    email_template_dto_envelope_local_var->error_code = error_code;
    email_template_dto_envelope_local_var->validation_details = validation_details;
...
void email_template_dto_envelope_free(email_template_dto_envelope_t *email_template_dto_envelope) {
    ...
    if (email_template_dto_envelope->error_code) {
        free(email_template_dto_envelope->error_code);
        email_template_dto_envelope->error_code = NULL;
    }
    if (email_template_dto_envelope->validation_details) {
        list_ForEach(listEntry, email_template_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(email_template_dto_envelope->validation_details);
        email_template_dto_envelope->validation_details = NULL;
    }
...
cJSON *email_template_dto_envelope_convertToJSON(email_template_dto_envelope_t *email_template_dto_envelope) {
    ...
    // email_template_dto_envelope->http_status
    if(email_template_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", email_template_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // email_template_dto_envelope->error_code
    if(email_template_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", email_template_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // email_template_dto_envelope->validation_details
    if(email_template_dto_envelope->validation_details) {
    ...
    if (email_template_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, email_template_dto_envelope->validation_details) {
...
email_template_dto_envelope_t *email_template_dto_envelope_parseFromJSON(cJSON *email_template_dto_envelopeJSON){
    ...
    // define the local map for email_template_dto_envelope->validation_details
    ...
    // email_template_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(email_template_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // email_template_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(email_template_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // email_template_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(email_template_dto_envelopeJSON, "validationDetails");
