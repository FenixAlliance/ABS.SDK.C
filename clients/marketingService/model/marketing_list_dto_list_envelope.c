...
marketing_list_dto_list_envelope_t *marketing_list_dto_list_envelope_create(
    ...
    marketing_list_dto_list_envelope_local_var->http_status = http_status;
    marketing_list_dto_list_envelope_local_var->error_code = error_code;
    marketing_list_dto_list_envelope_local_var->validation_details = validation_details;
...
void marketing_list_dto_list_envelope_free(marketing_list_dto_list_envelope_t *marketing_list_dto_list_envelope) {
    ...
    if (marketing_list_dto_list_envelope->error_code) {
        free(marketing_list_dto_list_envelope->error_code);
        marketing_list_dto_list_envelope->error_code = NULL;
    }
    if (marketing_list_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, marketing_list_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(marketing_list_dto_list_envelope->validation_details);
        marketing_list_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *marketing_list_dto_list_envelope_convertToJSON(marketing_list_dto_list_envelope_t *marketing_list_dto_list_envelope) {
    ...
    // marketing_list_dto_list_envelope->http_status
    if(marketing_list_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", marketing_list_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // marketing_list_dto_list_envelope->error_code
    if(marketing_list_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", marketing_list_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // marketing_list_dto_list_envelope->validation_details
    if(marketing_list_dto_list_envelope->validation_details) {
    ...
    if (marketing_list_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, marketing_list_dto_list_envelope->validation_details) {
...
marketing_list_dto_list_envelope_t *marketing_list_dto_list_envelope_parseFromJSON(cJSON *marketing_list_dto_list_envelopeJSON){
    ...
    // define the local map for marketing_list_dto_list_envelope->validation_details
    ...
    // marketing_list_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(marketing_list_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // marketing_list_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(marketing_list_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // marketing_list_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(marketing_list_dto_list_envelopeJSON, "validationDetails");
