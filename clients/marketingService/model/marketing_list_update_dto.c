...
#include <string.h>
#include <stdio.h>
#include "marketing_list_update_dto.h"
...
marketing_list_update_dto_t *marketing_list_update_dto_create(
    ...
    marketing_list_update_dto_t *marketing_list_update_dto_local_var = malloc(sizeof(marketing_list_update_dto_t));
    if (!marketing_list_update_dto_local_var) {
        return NULL;
    }
    marketing_list_update_dto_local_var->locked = locked;
    marketing_list_update_dto_local_var->name = name;
    marketing_list_update_dto_local_var->purpose = purpose;
    marketing_list_update_dto_local_var->description = description;
    marketing_list_update_dto_local_var->source = source;
    marketing_list_update_dto_local_var->cost = cost;
    ...
    return marketing_list_update_dto_local_var;
}


void marketing_list_update_dto_free(marketing_list_update_dto_t *marketing_list_update_dto) {
    if(NULL == marketing_list_update_dto){
        return ;
    }
    listEntry_t *listEntry;
    if (marketing_list_update_dto->name) {
        free(marketing_list_update_dto->name);
        marketing_list_update_dto->name = NULL;
    }
    if (marketing_list_update_dto->purpose) {
        free(marketing_list_update_dto->purpose);
        marketing_list_update_dto->purpose = NULL;
    }
    if (marketing_list_update_dto->description) {
        free(marketing_list_update_dto->description);
        marketing_list_update_dto->description = NULL;
    }
    if (marketing_list_update_dto->source) {
        free(marketing_list_update_dto->source);
        marketing_list_update_dto->source = NULL;
    }
    if (marketing_list_update_dto->modified_on) {
        free(marketing_list_update_dto->modified_on);
        marketing_list_update_dto->modified_on = NULL;
    }
    if (marketing_list_update_dto->last_used_on) {
        free(marketing_list_update_dto->last_used_on);
        marketing_list_update_dto->last_used_on = NULL;
    }
...
cJSON *marketing_list_update_dto_convertToJSON(marketing_list_update_dto_t *marketing_list_update_dto) {
    ...
    // marketing_list_update_dto->locked
    if(marketing_list_update_dto->locked) {
    if(cJSON_AddBoolToObject(item, "locked", marketing_list_update_dto->locked) == NULL) {
    goto fail; //Bool
    }
    }


    // marketing_list_update_dto->name
    if(marketing_list_update_dto->name) {
    if(cJSON_AddStringToObject(item, "name", marketing_list_update_dto->name) == NULL) {
    goto fail; //String
    }
    }


    // marketing_list_update_dto->purpose
    if(marketing_list_update_dto->purpose) {
    if(cJSON_AddStringToObject(item, "purpose", marketing_list_update_dto->purpose) == NULL) {
    goto fail; //String
    }
    }


    // marketing_list_update_dto->description
    if(marketing_list_update_dto->description) {
    if(cJSON_AddStringToObject(item, "description", marketing_list_update_dto->description) == NULL) {
    goto fail; //String
    }
    }


    // marketing_list_update_dto->source
    if(marketing_list_update_dto->source) {
    if(cJSON_AddStringToObject(item, "source", marketing_list_update_dto->source) == NULL) {
    goto fail; //String
    }
    }


    // marketing_list_update_dto->cost
    if(marketing_list_update_dto->cost) {
    if(cJSON_AddNumberToObject(item, "cost", marketing_list_update_dto->cost) == NULL) {
    goto fail; //Numeric
    }
    }


    // marketing_list_update_dto->modified_on
    if(marketing_list_update_dto->modified_on) {
    if(cJSON_AddStringToObject(item, "modifiedOn", marketing_list_update_dto->modified_on) == NULL) {
    goto fail; //Date-Time
    }
    }


    // marketing_list_update_dto->last_used_on
    if(marketing_list_update_dto->last_used_on) {
    if(cJSON_AddStringToObject(item, "lastUsedOn", marketing_list_update_dto->last_used_on) == NULL) {
    goto fail; //Date-Time
    }
    }
    ...
    if(marketing_list_update_dto->marketing_list_type != marketingservice_marketing_list_update_dto_MARKETINGLISTTYPE_NULL) {
    if(cJSON_AddStringToObject(item, "marketingListType", marketing_list_typemarketing_list_update_dto_ToString(marketing_list_update_dto->marketing_list_type)) == NULL)
    {
    goto fail; //Enum
    }
    }
    ...
    // marketing_list_update_dto->marketing_list_target
    if(marketing_list_update_dto->marketing_list_target != marketingservice_marketing_list_update_dto_MARKETINGLISTTARGET_NULL) {
    if(cJSON_AddStringToObject(item, "marketingListTarget", marketing_list_targetmarketing_list_update_dto_ToString(marketing_list_update_dto->marketing_list_target)) == NULL)
    {
    goto fail; //Enum
    }
    }
...
marketing_list_update_dto_t *marketing_list_update_dto_parseFromJSON(cJSON *marketing_list_update_dtoJSON){
    ...
    marketing_list_update_dto_t *marketing_list_update_dto_local_var = NULL;

    // marketing_list_update_dto->locked
    cJSON *locked = cJSON_GetObjectItemCaseSensitive(marketing_list_update_dtoJSON, "locked");
    if (locked) { 
    ...
    // marketing_list_update_dto->name
    cJSON *name = cJSON_GetObjectItemCaseSensitive(marketing_list_update_dtoJSON, "name");
    if (name) { 
    ...
    // marketing_list_update_dto->purpose
    cJSON *purpose = cJSON_GetObjectItemCaseSensitive(marketing_list_update_dtoJSON, "purpose");
    if (purpose) { 
    ...
    // marketing_list_update_dto->description
    cJSON *description = cJSON_GetObjectItemCaseSensitive(marketing_list_update_dtoJSON, "description");
    if (description) { 
    ...
    // marketing_list_update_dto->source
    cJSON *source = cJSON_GetObjectItemCaseSensitive(marketing_list_update_dtoJSON, "source");
    if (source) { 
    ...
    // marketing_list_update_dto->cost
    cJSON *cost = cJSON_GetObjectItemCaseSensitive(marketing_list_update_dtoJSON, "cost");
    if (cost) { 
    ...
    // marketing_list_update_dto->modified_on
    cJSON *modified_on = cJSON_GetObjectItemCaseSensitive(marketing_list_update_dtoJSON, "modifiedOn");
    if (modified_on) { 
    ...
    // marketing_list_update_dto->last_used_on
    cJSON *last_used_on = cJSON_GetObjectItemCaseSensitive(marketing_list_update_dtoJSON, "lastUsedOn");
    if (last_used_on) { 
    ...
    // marketing_list_update_dto->currency_id
    cJSON *currency_id = cJSON_GetObjectItemCaseSensitive(marketing_list_update_dtoJSON, "currencyId");
    if (currency_id) { 
    ...
    marketing_list_typeVariable = marketing_list_update_dto_marketing_list_type_FromString(marketing_list_type->valuestring);
    }

    // marketing_list_update_dto->marketing_list_target
    cJSON *marketing_list_target = cJSON_GetObjectItemCaseSensitive(marketing_list_update_dtoJSON, "marketingListTarget");
    marketingservice_marketing_list_update_dto_MARKETINGLISTTARGET_e marketing_list_targetVariable;
    if (marketing_list_target) { 
    if(!cJSON_IsString(marketing_list_target))
    {
    goto end; //Enum
    }
    marketing_list_targetVariable = marketing_list_update_dto_marketing_list_target_FromString(marketing_list_target->valuestring);
    }


    marketing_list_update_dto_local_var = marketing_list_update_dto_create (
