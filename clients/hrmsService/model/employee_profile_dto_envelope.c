...
employee_profile_dto_envelope_t *employee_profile_dto_envelope_create(
    ...
    employee_profile_dto_envelope_local_var->http_status = http_status;
    employee_profile_dto_envelope_local_var->error_code = error_code;
    employee_profile_dto_envelope_local_var->validation_details = validation_details;
...
void employee_profile_dto_envelope_free(employee_profile_dto_envelope_t *employee_profile_dto_envelope) {
    ...
    if (employee_profile_dto_envelope->error_code) {
        free(employee_profile_dto_envelope->error_code);
        employee_profile_dto_envelope->error_code = NULL;
    }
    if (employee_profile_dto_envelope->validation_details) {
        list_ForEach(listEntry, employee_profile_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(employee_profile_dto_envelope->validation_details);
        employee_profile_dto_envelope->validation_details = NULL;
    }
...
cJSON *employee_profile_dto_envelope_convertToJSON(employee_profile_dto_envelope_t *employee_profile_dto_envelope) {
    ...
    // employee_profile_dto_envelope->http_status
    if(employee_profile_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", employee_profile_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // employee_profile_dto_envelope->error_code
    if(employee_profile_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", employee_profile_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // employee_profile_dto_envelope->validation_details
    if(employee_profile_dto_envelope->validation_details) {
    ...
    if (employee_profile_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, employee_profile_dto_envelope->validation_details) {
...
employee_profile_dto_envelope_t *employee_profile_dto_envelope_parseFromJSON(cJSON *employee_profile_dto_envelopeJSON){
    ...
    // define the local map for employee_profile_dto_envelope->validation_details
    ...
    // employee_profile_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(employee_profile_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // employee_profile_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(employee_profile_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // employee_profile_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(employee_profile_dto_envelopeJSON, "validationDetails");
