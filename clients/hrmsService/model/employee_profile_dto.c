...
void employee_profile_dto_free(employee_profile_dto_t *employee_profile_dto) {
    if(NULL == employee_profile_dto){
        return ;
    }
    listEntry_t *listEntry;
    if (employee_profile_dto->id) {
        free(employee_profile_dto->id);
        employee_profile_dto->id = NULL;
    }
...
cJSON *employee_profile_dto_convertToJSON(employee_profile_dto_t *employee_profile_dto) {
    ...
    if(employee_profile_dto->id) {
    if(cJSON_AddStringToObject(item, "id", employee_profile_dto->id) == NULL) {
    goto fail; //String
    }
    }


    // employee_profile_dto->timestamp
    if(employee_profile_dto->timestamp) {
    if(cJSON_AddStringToObject(item, "timestamp", employee_profile_dto->timestamp) == NULL) {
    goto fail; //Date-Time
    }
    }
...
employee_profile_dto_t *employee_profile_dto_parseFromJSON(cJSON *employee_profile_dtoJSON){
    ...
    // employee_profile_dto->timestamp
    cJSON *timestamp = cJSON_GetObjectItemCaseSensitive(employee_profile_dtoJSON, "timestamp");
    if (timestamp) { 
