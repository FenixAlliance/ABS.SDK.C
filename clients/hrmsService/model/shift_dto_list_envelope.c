...
shift_dto_list_envelope_t *shift_dto_list_envelope_create(
    ...
    shift_dto_list_envelope_local_var->http_status = http_status;
    shift_dto_list_envelope_local_var->error_code = error_code;
    shift_dto_list_envelope_local_var->validation_details = validation_details;
...
void shift_dto_list_envelope_free(shift_dto_list_envelope_t *shift_dto_list_envelope) {
    ...
    if (shift_dto_list_envelope->error_code) {
        free(shift_dto_list_envelope->error_code);
        shift_dto_list_envelope->error_code = NULL;
    }
    if (shift_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, shift_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(shift_dto_list_envelope->validation_details);
        shift_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *shift_dto_list_envelope_convertToJSON(shift_dto_list_envelope_t *shift_dto_list_envelope) {
    ...
    // shift_dto_list_envelope->http_status
    if(shift_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", shift_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // shift_dto_list_envelope->error_code
    if(shift_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", shift_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // shift_dto_list_envelope->validation_details
    if(shift_dto_list_envelope->validation_details) {
    ...
    if (shift_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, shift_dto_list_envelope->validation_details) {
...
shift_dto_list_envelope_t *shift_dto_list_envelope_parseFromJSON(cJSON *shift_dto_list_envelopeJSON){
    ...
    // define the local map for shift_dto_list_envelope->validation_details
    ...
    // shift_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(shift_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // shift_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(shift_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // shift_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(shift_dto_list_envelopeJSON, "validationDetails");
