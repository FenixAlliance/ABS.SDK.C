...
salary_dto_envelope_t *salary_dto_envelope_create(
    ...
    salary_dto_envelope_local_var->http_status = http_status;
    salary_dto_envelope_local_var->error_code = error_code;
    salary_dto_envelope_local_var->validation_details = validation_details;
...
void salary_dto_envelope_free(salary_dto_envelope_t *salary_dto_envelope) {
    ...
    if (salary_dto_envelope->error_code) {
        free(salary_dto_envelope->error_code);
        salary_dto_envelope->error_code = NULL;
    }
    if (salary_dto_envelope->validation_details) {
        list_ForEach(listEntry, salary_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(salary_dto_envelope->validation_details);
        salary_dto_envelope->validation_details = NULL;
    }
...
cJSON *salary_dto_envelope_convertToJSON(salary_dto_envelope_t *salary_dto_envelope) {
    ...
    // salary_dto_envelope->http_status
    if(salary_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", salary_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // salary_dto_envelope->error_code
    if(salary_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", salary_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // salary_dto_envelope->validation_details
    if(salary_dto_envelope->validation_details) {
    ...
    if (salary_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, salary_dto_envelope->validation_details) {
...
salary_dto_envelope_t *salary_dto_envelope_parseFromJSON(cJSON *salary_dto_envelopeJSON){
    ...
    // define the local map for salary_dto_envelope->validation_details
    ...
    // salary_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(salary_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // salary_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(salary_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // salary_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(salary_dto_envelopeJSON, "validationDetails");
