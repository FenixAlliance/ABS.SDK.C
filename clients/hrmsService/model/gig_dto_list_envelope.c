...
gig_dto_list_envelope_t *gig_dto_list_envelope_create(
    ...
    gig_dto_list_envelope_local_var->http_status = http_status;
    gig_dto_list_envelope_local_var->error_code = error_code;
    gig_dto_list_envelope_local_var->validation_details = validation_details;
...
void gig_dto_list_envelope_free(gig_dto_list_envelope_t *gig_dto_list_envelope) {
    ...
    if (gig_dto_list_envelope->error_code) {
        free(gig_dto_list_envelope->error_code);
        gig_dto_list_envelope->error_code = NULL;
    }
    if (gig_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, gig_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(gig_dto_list_envelope->validation_details);
        gig_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *gig_dto_list_envelope_convertToJSON(gig_dto_list_envelope_t *gig_dto_list_envelope) {
    ...
    // gig_dto_list_envelope->http_status
    if(gig_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", gig_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // gig_dto_list_envelope->error_code
    if(gig_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", gig_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // gig_dto_list_envelope->validation_details
    if(gig_dto_list_envelope->validation_details) {
    ...
    if (gig_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, gig_dto_list_envelope->validation_details) {
...
gig_dto_list_envelope_t *gig_dto_list_envelope_parseFromJSON(cJSON *gig_dto_list_envelopeJSON){
    ...
    // define the local map for gig_dto_list_envelope->validation_details
    ...
    // gig_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(gig_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // gig_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(gig_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // gig_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(gig_dto_list_envelopeJSON, "validationDetails");
