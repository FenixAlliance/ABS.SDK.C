...
payroll_dto_list_envelope_t *payroll_dto_list_envelope_create(
    ...
    payroll_dto_list_envelope_local_var->http_status = http_status;
    payroll_dto_list_envelope_local_var->error_code = error_code;
    payroll_dto_list_envelope_local_var->validation_details = validation_details;
...
void payroll_dto_list_envelope_free(payroll_dto_list_envelope_t *payroll_dto_list_envelope) {
    ...
    if (payroll_dto_list_envelope->error_code) {
        free(payroll_dto_list_envelope->error_code);
        payroll_dto_list_envelope->error_code = NULL;
    }
    if (payroll_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, payroll_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(payroll_dto_list_envelope->validation_details);
        payroll_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *payroll_dto_list_envelope_convertToJSON(payroll_dto_list_envelope_t *payroll_dto_list_envelope) {
    ...
    // payroll_dto_list_envelope->http_status
    if(payroll_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", payroll_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // payroll_dto_list_envelope->error_code
    if(payroll_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", payroll_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // payroll_dto_list_envelope->validation_details
    if(payroll_dto_list_envelope->validation_details) {
    ...
    if (payroll_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, payroll_dto_list_envelope->validation_details) {
...
payroll_dto_list_envelope_t *payroll_dto_list_envelope_parseFromJSON(cJSON *payroll_dto_list_envelopeJSON){
    ...
    // define the local map for payroll_dto_list_envelope->validation_details
    ...
    // payroll_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(payroll_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // payroll_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(payroll_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // payroll_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(payroll_dto_list_envelopeJSON, "validationDetails");
