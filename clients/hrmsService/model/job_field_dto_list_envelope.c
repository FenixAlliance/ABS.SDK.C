...
job_field_dto_list_envelope_t *job_field_dto_list_envelope_create(
    ...
    job_field_dto_list_envelope_local_var->http_status = http_status;
    job_field_dto_list_envelope_local_var->error_code = error_code;
    job_field_dto_list_envelope_local_var->validation_details = validation_details;
...
void job_field_dto_list_envelope_free(job_field_dto_list_envelope_t *job_field_dto_list_envelope) {
    ...
    if (job_field_dto_list_envelope->error_code) {
        free(job_field_dto_list_envelope->error_code);
        job_field_dto_list_envelope->error_code = NULL;
    }
    if (job_field_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, job_field_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(job_field_dto_list_envelope->validation_details);
        job_field_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *job_field_dto_list_envelope_convertToJSON(job_field_dto_list_envelope_t *job_field_dto_list_envelope) {
    ...
    // job_field_dto_list_envelope->http_status
    if(job_field_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", job_field_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // job_field_dto_list_envelope->error_code
    if(job_field_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", job_field_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // job_field_dto_list_envelope->validation_details
    if(job_field_dto_list_envelope->validation_details) {
    ...
    if (job_field_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, job_field_dto_list_envelope->validation_details) {
...
job_field_dto_list_envelope_t *job_field_dto_list_envelope_parseFromJSON(cJSON *job_field_dto_list_envelopeJSON){
    ...
    // define the local map for job_field_dto_list_envelope->validation_details
    ...
    // job_field_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(job_field_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // job_field_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(job_field_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // job_field_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(job_field_dto_list_envelopeJSON, "validationDetails");
