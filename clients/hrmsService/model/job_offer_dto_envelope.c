...
job_offer_dto_envelope_t *job_offer_dto_envelope_create(
    ...
    job_offer_dto_envelope_local_var->http_status = http_status;
    job_offer_dto_envelope_local_var->error_code = error_code;
    job_offer_dto_envelope_local_var->validation_details = validation_details;
...
void job_offer_dto_envelope_free(job_offer_dto_envelope_t *job_offer_dto_envelope) {
    ...
    if (job_offer_dto_envelope->error_code) {
        free(job_offer_dto_envelope->error_code);
        job_offer_dto_envelope->error_code = NULL;
    }
    if (job_offer_dto_envelope->validation_details) {
        list_ForEach(listEntry, job_offer_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(job_offer_dto_envelope->validation_details);
        job_offer_dto_envelope->validation_details = NULL;
    }
...
cJSON *job_offer_dto_envelope_convertToJSON(job_offer_dto_envelope_t *job_offer_dto_envelope) {
    ...
    // job_offer_dto_envelope->http_status
    if(job_offer_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", job_offer_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // job_offer_dto_envelope->error_code
    if(job_offer_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", job_offer_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // job_offer_dto_envelope->validation_details
    if(job_offer_dto_envelope->validation_details) {
    ...
    if (job_offer_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, job_offer_dto_envelope->validation_details) {
...
job_offer_dto_envelope_t *job_offer_dto_envelope_parseFromJSON(cJSON *job_offer_dto_envelopeJSON){
    ...
    // define the local map for job_offer_dto_envelope->validation_details
    ...
    // job_offer_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(job_offer_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // job_offer_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(job_offer_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // job_offer_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(job_offer_dto_envelopeJSON, "validationDetails");
