/*
 * job_applicant_profile_dto_envelope.h
 *
 * 
 */

#ifndef _job_applicant_profile_dto_envelope_H_
#define _job_applicant_profile_dto_envelope_H_

#include <string.h>
#include "../external/cJSON.h"
#include "../include/list.h"
#include "../include/keyValuePair.h"
#include "../include/binary.h"

typedef struct job_applicant_profile_dto_envelope_t job_applicant_profile_dto_envelope_t;

#include "job_applicant_profile_dto.h"



typedef struct job_applicant_profile_dto_envelope_t {
    int is_success; //boolean
    char *error_message; // string
    char *correlation_id; // string
    char *timestamp; //date time
    char *activity_id; // string
    struct job_applicant_profile_dto_t *result; //model

} job_applicant_profile_dto_envelope_t;

job_applicant_profile_dto_envelope_t *job_applicant_profile_dto_envelope_create(
    int is_success,
    char *error_message,
    char *correlation_id,
    char *timestamp,
    char *activity_id,
    job_applicant_profile_dto_t *result
);

void job_applicant_profile_dto_envelope_free(job_applicant_profile_dto_envelope_t *job_applicant_profile_dto_envelope);

job_applicant_profile_dto_envelope_t *job_applicant_profile_dto_envelope_parseFromJSON(cJSON *job_applicant_profile_dto_envelopeJSON);

cJSON *job_applicant_profile_dto_envelope_convertToJSON(job_applicant_profile_dto_envelope_t *job_applicant_profile_dto_envelope);

#endif /* _job_applicant_profile_dto_envelope_H_ */

