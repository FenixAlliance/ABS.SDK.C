...
leave_application_dto_envelope_t *leave_application_dto_envelope_create(
    ...
    leave_application_dto_envelope_local_var->http_status = http_status;
    leave_application_dto_envelope_local_var->error_code = error_code;
    leave_application_dto_envelope_local_var->validation_details = validation_details;
...
void leave_application_dto_envelope_free(leave_application_dto_envelope_t *leave_application_dto_envelope) {
    ...
    if (leave_application_dto_envelope->error_code) {
        free(leave_application_dto_envelope->error_code);
        leave_application_dto_envelope->error_code = NULL;
    }
    if (leave_application_dto_envelope->validation_details) {
        list_ForEach(listEntry, leave_application_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(leave_application_dto_envelope->validation_details);
        leave_application_dto_envelope->validation_details = NULL;
    }
...
cJSON *leave_application_dto_envelope_convertToJSON(leave_application_dto_envelope_t *leave_application_dto_envelope) {
    ...
    // leave_application_dto_envelope->http_status
    if(leave_application_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", leave_application_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // leave_application_dto_envelope->error_code
    if(leave_application_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", leave_application_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // leave_application_dto_envelope->validation_details
    if(leave_application_dto_envelope->validation_details) {
    ...
    if (leave_application_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, leave_application_dto_envelope->validation_details) {
...
leave_application_dto_envelope_t *leave_application_dto_envelope_parseFromJSON(cJSON *leave_application_dto_envelopeJSON){
    ...
    // define the local map for leave_application_dto_envelope->validation_details
    ...
    // leave_application_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(leave_application_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // leave_application_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(leave_application_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // leave_application_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(leave_application_dto_envelopeJSON, "validationDetails");
