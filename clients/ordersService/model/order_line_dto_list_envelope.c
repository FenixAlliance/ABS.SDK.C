...
order_line_dto_list_envelope_t *order_line_dto_list_envelope_create(
    ...
    order_line_dto_list_envelope_local_var->http_status = http_status;
    order_line_dto_list_envelope_local_var->error_code = error_code;
    order_line_dto_list_envelope_local_var->validation_details = validation_details;
...
void order_line_dto_list_envelope_free(order_line_dto_list_envelope_t *order_line_dto_list_envelope) {
    ...
    if (order_line_dto_list_envelope->error_code) {
        free(order_line_dto_list_envelope->error_code);
        order_line_dto_list_envelope->error_code = NULL;
    }
    if (order_line_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, order_line_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(order_line_dto_list_envelope->validation_details);
        order_line_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *order_line_dto_list_envelope_convertToJSON(order_line_dto_list_envelope_t *order_line_dto_list_envelope) {
    ...
    // order_line_dto_list_envelope->http_status
    if(order_line_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", order_line_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // order_line_dto_list_envelope->error_code
    if(order_line_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", order_line_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // order_line_dto_list_envelope->validation_details
    if(order_line_dto_list_envelope->validation_details) {
    ...
    if (order_line_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, order_line_dto_list_envelope->validation_details) {
...
order_line_dto_list_envelope_t *order_line_dto_list_envelope_parseFromJSON(cJSON *order_line_dto_list_envelopeJSON){
    ...
    // define the local map for order_line_dto_list_envelope->validation_details
    ...
    // order_line_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(order_line_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // order_line_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(order_line_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // order_line_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(order_line_dto_list_envelopeJSON, "validationDetails");
