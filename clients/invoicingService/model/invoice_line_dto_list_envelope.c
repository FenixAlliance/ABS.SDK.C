...
invoice_line_dto_list_envelope_t *invoice_line_dto_list_envelope_create(
    ...
    invoice_line_dto_list_envelope_local_var->http_status = http_status;
    invoice_line_dto_list_envelope_local_var->error_code = error_code;
    invoice_line_dto_list_envelope_local_var->validation_details = validation_details;
...
void invoice_line_dto_list_envelope_free(invoice_line_dto_list_envelope_t *invoice_line_dto_list_envelope) {
    ...
    if (invoice_line_dto_list_envelope->error_code) {
        free(invoice_line_dto_list_envelope->error_code);
        invoice_line_dto_list_envelope->error_code = NULL;
    }
    if (invoice_line_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, invoice_line_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(invoice_line_dto_list_envelope->validation_details);
        invoice_line_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *invoice_line_dto_list_envelope_convertToJSON(invoice_line_dto_list_envelope_t *invoice_line_dto_list_envelope) {
    ...
    // invoice_line_dto_list_envelope->http_status
    if(invoice_line_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", invoice_line_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // invoice_line_dto_list_envelope->error_code
    if(invoice_line_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", invoice_line_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // invoice_line_dto_list_envelope->validation_details
    if(invoice_line_dto_list_envelope->validation_details) {
    ...
    if (invoice_line_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, invoice_line_dto_list_envelope->validation_details) {
...
invoice_line_dto_list_envelope_t *invoice_line_dto_list_envelope_parseFromJSON(cJSON *invoice_line_dto_list_envelopeJSON){
    ...
    // define the local map for invoice_line_dto_list_envelope->validation_details
    ...
    // invoice_line_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(invoice_line_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // invoice_line_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(invoice_line_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // invoice_line_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(invoice_line_dto_list_envelopeJSON, "validationDetails");
