...
money_envelope_t *money_envelope_create(
    ...
    money_envelope_local_var->http_status = http_status;
    money_envelope_local_var->error_code = error_code;
    money_envelope_local_var->validation_details = validation_details;
...
void money_envelope_free(money_envelope_t *money_envelope) {
    ...
    if (money_envelope->error_code) {
        free(money_envelope->error_code);
        money_envelope->error_code = NULL;
    }
    if (money_envelope->validation_details) {
        list_ForEach(listEntry, money_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(money_envelope->validation_details);
        money_envelope->validation_details = NULL;
    }
...
cJSON *money_envelope_convertToJSON(money_envelope_t *money_envelope) {
    ...
    // money_envelope->http_status
    if(money_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", money_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // money_envelope->error_code
    if(money_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", money_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // money_envelope->validation_details
    if(money_envelope->validation_details) {
    ...
    if (money_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, money_envelope->validation_details) {
...
money_envelope_t *money_envelope_parseFromJSON(cJSON *money_envelopeJSON){
    ...
    // define the local map for money_envelope->validation_details
    ...
    // money_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(money_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // money_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(money_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // money_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(money_envelopeJSON, "validationDetails");
