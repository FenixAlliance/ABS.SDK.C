...
extended_deal_unit_dto_list_envelope_t *extended_deal_unit_dto_list_envelope_create(
    ...
    extended_deal_unit_dto_list_envelope_local_var->http_status = http_status;
    extended_deal_unit_dto_list_envelope_local_var->error_code = error_code;
    extended_deal_unit_dto_list_envelope_local_var->validation_details = validation_details;
...
void extended_deal_unit_dto_list_envelope_free(extended_deal_unit_dto_list_envelope_t *extended_deal_unit_dto_list_envelope) {
    ...
    if (extended_deal_unit_dto_list_envelope->error_code) {
        free(extended_deal_unit_dto_list_envelope->error_code);
        extended_deal_unit_dto_list_envelope->error_code = NULL;
    }
    if (extended_deal_unit_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, extended_deal_unit_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(extended_deal_unit_dto_list_envelope->validation_details);
        extended_deal_unit_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *extended_deal_unit_dto_list_envelope_convertToJSON(extended_deal_unit_dto_list_envelope_t *extended_deal_unit_dto_list_envelope) {
    ...
    // extended_deal_unit_dto_list_envelope->http_status
    if(extended_deal_unit_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", extended_deal_unit_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // extended_deal_unit_dto_list_envelope->error_code
    if(extended_deal_unit_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", extended_deal_unit_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // extended_deal_unit_dto_list_envelope->validation_details
    if(extended_deal_unit_dto_list_envelope->validation_details) {
    ...
    if (extended_deal_unit_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, extended_deal_unit_dto_list_envelope->validation_details) {
...
extended_deal_unit_dto_list_envelope_t *extended_deal_unit_dto_list_envelope_parseFromJSON(cJSON *extended_deal_unit_dto_list_envelopeJSON){
    ...
    // define the local map for extended_deal_unit_dto_list_envelope->validation_details
    ...
    // extended_deal_unit_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(extended_deal_unit_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // extended_deal_unit_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(extended_deal_unit_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // extended_deal_unit_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(extended_deal_unit_dto_list_envelopeJSON, "validationDetails");
