...
deal_unit_flow_dto_list_envelope_t *deal_unit_flow_dto_list_envelope_create(
    ...
    deal_unit_flow_dto_list_envelope_local_var->http_status = http_status;
    deal_unit_flow_dto_list_envelope_local_var->error_code = error_code;
    deal_unit_flow_dto_list_envelope_local_var->validation_details = validation_details;
...
void deal_unit_flow_dto_list_envelope_free(deal_unit_flow_dto_list_envelope_t *deal_unit_flow_dto_list_envelope) {
    ...
    if (deal_unit_flow_dto_list_envelope->error_code) {
        free(deal_unit_flow_dto_list_envelope->error_code);
        deal_unit_flow_dto_list_envelope->error_code = NULL;
    }
    if (deal_unit_flow_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, deal_unit_flow_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(deal_unit_flow_dto_list_envelope->validation_details);
        deal_unit_flow_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *deal_unit_flow_dto_list_envelope_convertToJSON(deal_unit_flow_dto_list_envelope_t *deal_unit_flow_dto_list_envelope) {
    ...
    // deal_unit_flow_dto_list_envelope->http_status
    if(deal_unit_flow_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", deal_unit_flow_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // deal_unit_flow_dto_list_envelope->error_code
    if(deal_unit_flow_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", deal_unit_flow_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // deal_unit_flow_dto_list_envelope->validation_details
    if(deal_unit_flow_dto_list_envelope->validation_details) {
    ...
    if (deal_unit_flow_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, deal_unit_flow_dto_list_envelope->validation_details) {
...
deal_unit_flow_dto_list_envelope_t *deal_unit_flow_dto_list_envelope_parseFromJSON(cJSON *deal_unit_flow_dto_list_envelopeJSON){
    ...
    // define the local map for deal_unit_flow_dto_list_envelope->validation_details
    ...
    // deal_unit_flow_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(deal_unit_flow_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // deal_unit_flow_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(deal_unit_flow_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // deal_unit_flow_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(deal_unit_flow_dto_list_envelopeJSON, "validationDetails");
