...
course_enrollment_dto_t *course_enrollment_dto_create(
    char *id,
    char *timestamp,
    char *course_id,
    ...
    if (!course_enrollment_dto_local_var) {
        return NULL;
    }
    course_enrollment_dto_local_var->id = id;
    course_enrollment_dto_local_var->timestamp = timestamp;
...
void course_enrollment_dto_free(course_enrollment_dto_t *course_enrollment_dto) {
    if(NULL == course_enrollment_dto){
        return ;
    }
    listEntry_t *listEntry;
    if (course_enrollment_dto->id) {
        free(course_enrollment_dto->id);
        course_enrollment_dto->id = NULL;
    }
    if (course_enrollment_dto->timestamp) {
        free(course_enrollment_dto->timestamp);
        course_enrollment_dto->timestamp = NULL;
    }
    if (course_enrollment_dto->course_id) {
        free(course_enrollment_dto->course_id);
        course_enrollment_dto->course_id = NULL;
    }
    if (course_enrollment_dto->course_cohort_id) {
        free(course_enrollment_dto->course_cohort_id);
        course_enrollment_dto->course_cohort_id = NULL;
    }
    ...
    if (course_enrollment_dto->course_completion_certificate_id) {
        ...
        course_enrollment_dto->course_completion_certificate_id = NULL;
    }
    free(course_enrollment_dto);
}

cJSON *course_enrollment_dto_convertToJSON(course_enrollment_dto_t *course_enrollment_dto) {
    cJSON *item = cJSON_CreateObject();

    // course_enrollment_dto->id
    if(course_enrollment_dto->id) {
    if(cJSON_AddStringToObject(item, "id", course_enrollment_dto->id) == NULL) {
    goto fail; //String
    }
    }


    // course_enrollment_dto->timestamp
    if(course_enrollment_dto->timestamp) {
    if(cJSON_AddStringToObject(item, "timestamp", course_enrollment_dto->timestamp) == NULL) {
    goto fail; //Date-Time
    }
    }
...
course_enrollment_dto_t *course_enrollment_dto_parseFromJSON(cJSON *course_enrollment_dtoJSON){

    course_enrollment_dto_t *course_enrollment_dto_local_var = NULL;

    // course_enrollment_dto->id
    cJSON *id = cJSON_GetObjectItemCaseSensitive(course_enrollment_dtoJSON, "id");
    if (id) { 
    ...
    // course_enrollment_dto->timestamp
    cJSON *timestamp = cJSON_GetObjectItemCaseSensitive(course_enrollment_dtoJSON, "timestamp");
    if (timestamp) { 
    ...
    course_enrollment_dto_local_var = course_enrollment_dto_create (
