...
support_ticket_type_dto_list_envelope_t *support_ticket_type_dto_list_envelope_create(
    ...
    support_ticket_type_dto_list_envelope_local_var->http_status = http_status;
    support_ticket_type_dto_list_envelope_local_var->error_code = error_code;
    support_ticket_type_dto_list_envelope_local_var->validation_details = validation_details;
...
void support_ticket_type_dto_list_envelope_free(support_ticket_type_dto_list_envelope_t *support_ticket_type_dto_list_envelope) {
    ...
    if (support_ticket_type_dto_list_envelope->error_code) {
        free(support_ticket_type_dto_list_envelope->error_code);
        support_ticket_type_dto_list_envelope->error_code = NULL;
    }
    if (support_ticket_type_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, support_ticket_type_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(support_ticket_type_dto_list_envelope->validation_details);
        support_ticket_type_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *support_ticket_type_dto_list_envelope_convertToJSON(support_ticket_type_dto_list_envelope_t *support_ticket_type_dto_list_envelope) {
    ...
    // support_ticket_type_dto_list_envelope->http_status
    if(support_ticket_type_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", support_ticket_type_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // support_ticket_type_dto_list_envelope->error_code
    if(support_ticket_type_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", support_ticket_type_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // support_ticket_type_dto_list_envelope->validation_details
    if(support_ticket_type_dto_list_envelope->validation_details) {
    ...
    if (support_ticket_type_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, support_ticket_type_dto_list_envelope->validation_details) {
...
support_ticket_type_dto_list_envelope_t *support_ticket_type_dto_list_envelope_parseFromJSON(cJSON *support_ticket_type_dto_list_envelopeJSON){
    ...
    // define the local map for support_ticket_type_dto_list_envelope->validation_details
    ...
    // support_ticket_type_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(support_ticket_type_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // support_ticket_type_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(support_ticket_type_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // support_ticket_type_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(support_ticket_type_dto_list_envelopeJSON, "validationDetails");
