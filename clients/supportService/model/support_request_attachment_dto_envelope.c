...
support_request_attachment_dto_envelope_t *support_request_attachment_dto_envelope_create(
    ...
    support_request_attachment_dto_envelope_local_var->http_status = http_status;
    support_request_attachment_dto_envelope_local_var->error_code = error_code;
    support_request_attachment_dto_envelope_local_var->validation_details = validation_details;
...
void support_request_attachment_dto_envelope_free(support_request_attachment_dto_envelope_t *support_request_attachment_dto_envelope) {
    ...
    if (support_request_attachment_dto_envelope->error_code) {
        free(support_request_attachment_dto_envelope->error_code);
        support_request_attachment_dto_envelope->error_code = NULL;
    }
    if (support_request_attachment_dto_envelope->validation_details) {
        list_ForEach(listEntry, support_request_attachment_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(support_request_attachment_dto_envelope->validation_details);
        support_request_attachment_dto_envelope->validation_details = NULL;
    }
...
cJSON *support_request_attachment_dto_envelope_convertToJSON(support_request_attachment_dto_envelope_t *support_request_attachment_dto_envelope) {
    ...
    // support_request_attachment_dto_envelope->http_status
    if(support_request_attachment_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", support_request_attachment_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // support_request_attachment_dto_envelope->error_code
    if(support_request_attachment_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", support_request_attachment_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // support_request_attachment_dto_envelope->validation_details
    if(support_request_attachment_dto_envelope->validation_details) {
    ...
    if (support_request_attachment_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, support_request_attachment_dto_envelope->validation_details) {
...
support_request_attachment_dto_envelope_t *support_request_attachment_dto_envelope_parseFromJSON(cJSON *support_request_attachment_dto_envelopeJSON){
    ...
    // define the local map for support_request_attachment_dto_envelope->validation_details
    ...
    // support_request_attachment_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(support_request_attachment_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // support_request_attachment_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(support_request_attachment_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // support_request_attachment_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(support_request_attachment_dto_envelopeJSON, "validationDetails");
