...
warranty_request_dto_list_envelope_t *warranty_request_dto_list_envelope_create(
    ...
    warranty_request_dto_list_envelope_local_var->http_status = http_status;
    warranty_request_dto_list_envelope_local_var->error_code = error_code;
    warranty_request_dto_list_envelope_local_var->validation_details = validation_details;
...
void warranty_request_dto_list_envelope_free(warranty_request_dto_list_envelope_t *warranty_request_dto_list_envelope) {
    ...
    if (warranty_request_dto_list_envelope->error_code) {
        free(warranty_request_dto_list_envelope->error_code);
        warranty_request_dto_list_envelope->error_code = NULL;
    }
    if (warranty_request_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, warranty_request_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(warranty_request_dto_list_envelope->validation_details);
        warranty_request_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *warranty_request_dto_list_envelope_convertToJSON(warranty_request_dto_list_envelope_t *warranty_request_dto_list_envelope) {
    ...
    // warranty_request_dto_list_envelope->http_status
    if(warranty_request_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", warranty_request_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // warranty_request_dto_list_envelope->error_code
    if(warranty_request_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", warranty_request_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // warranty_request_dto_list_envelope->validation_details
    if(warranty_request_dto_list_envelope->validation_details) {
    ...
    if (warranty_request_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, warranty_request_dto_list_envelope->validation_details) {
...
warranty_request_dto_list_envelope_t *warranty_request_dto_list_envelope_parseFromJSON(cJSON *warranty_request_dto_list_envelopeJSON){
    ...
    // define the local map for warranty_request_dto_list_envelope->validation_details
    ...
    // warranty_request_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(warranty_request_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // warranty_request_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(warranty_request_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // warranty_request_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(warranty_request_dto_list_envelopeJSON, "validationDetails");
