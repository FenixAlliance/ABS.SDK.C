...
inquiry_request_dto_envelope_t *inquiry_request_dto_envelope_create(
    ...
    inquiry_request_dto_envelope_local_var->http_status = http_status;
    inquiry_request_dto_envelope_local_var->error_code = error_code;
    inquiry_request_dto_envelope_local_var->validation_details = validation_details;
...
void inquiry_request_dto_envelope_free(inquiry_request_dto_envelope_t *inquiry_request_dto_envelope) {
    ...
    if (inquiry_request_dto_envelope->error_code) {
        free(inquiry_request_dto_envelope->error_code);
        inquiry_request_dto_envelope->error_code = NULL;
    }
    if (inquiry_request_dto_envelope->validation_details) {
        list_ForEach(listEntry, inquiry_request_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(inquiry_request_dto_envelope->validation_details);
        inquiry_request_dto_envelope->validation_details = NULL;
    }
...
cJSON *inquiry_request_dto_envelope_convertToJSON(inquiry_request_dto_envelope_t *inquiry_request_dto_envelope) {
    ...
    // inquiry_request_dto_envelope->http_status
    if(inquiry_request_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", inquiry_request_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // inquiry_request_dto_envelope->error_code
    if(inquiry_request_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", inquiry_request_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // inquiry_request_dto_envelope->validation_details
    if(inquiry_request_dto_envelope->validation_details) {
    ...
    if (inquiry_request_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, inquiry_request_dto_envelope->validation_details) {
...
inquiry_request_dto_envelope_t *inquiry_request_dto_envelope_parseFromJSON(cJSON *inquiry_request_dto_envelopeJSON){
    ...
    // define the local map for inquiry_request_dto_envelope->validation_details
    ...
    // inquiry_request_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(inquiry_request_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // inquiry_request_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(inquiry_request_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // inquiry_request_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(inquiry_request_dto_envelopeJSON, "validationDetails");
