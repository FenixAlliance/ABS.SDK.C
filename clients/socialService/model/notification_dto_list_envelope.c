...
notification_dto_list_envelope_t *notification_dto_list_envelope_create(
    ...
    notification_dto_list_envelope_local_var->http_status = http_status;
    notification_dto_list_envelope_local_var->error_code = error_code;
    notification_dto_list_envelope_local_var->validation_details = validation_details;
...
void notification_dto_list_envelope_free(notification_dto_list_envelope_t *notification_dto_list_envelope) {
    ...
    if (notification_dto_list_envelope->error_code) {
        free(notification_dto_list_envelope->error_code);
        notification_dto_list_envelope->error_code = NULL;
    }
    if (notification_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, notification_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(notification_dto_list_envelope->validation_details);
        notification_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *notification_dto_list_envelope_convertToJSON(notification_dto_list_envelope_t *notification_dto_list_envelope) {
    ...
    // notification_dto_list_envelope->http_status
    if(notification_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", notification_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // notification_dto_list_envelope->error_code
    if(notification_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", notification_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // notification_dto_list_envelope->validation_details
    if(notification_dto_list_envelope->validation_details) {
    ...
    if (notification_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, notification_dto_list_envelope->validation_details) {
...
notification_dto_list_envelope_t *notification_dto_list_envelope_parseFromJSON(cJSON *notification_dto_list_envelopeJSON){
    ...
    // define the local map for notification_dto_list_envelope->validation_details
    ...
    // notification_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(notification_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // notification_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(notification_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // notification_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(notification_dto_list_envelopeJSON, "validationDetails");
