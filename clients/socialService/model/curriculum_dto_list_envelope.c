...
curriculum_dto_list_envelope_t *curriculum_dto_list_envelope_create(
    ...
    curriculum_dto_list_envelope_local_var->http_status = http_status;
    curriculum_dto_list_envelope_local_var->error_code = error_code;
    curriculum_dto_list_envelope_local_var->validation_details = validation_details;
...
void curriculum_dto_list_envelope_free(curriculum_dto_list_envelope_t *curriculum_dto_list_envelope) {
    ...
    if (curriculum_dto_list_envelope->error_code) {
        free(curriculum_dto_list_envelope->error_code);
        curriculum_dto_list_envelope->error_code = NULL;
    }
    if (curriculum_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, curriculum_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(curriculum_dto_list_envelope->validation_details);
        curriculum_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *curriculum_dto_list_envelope_convertToJSON(curriculum_dto_list_envelope_t *curriculum_dto_list_envelope) {
    ...
    // curriculum_dto_list_envelope->http_status
    if(curriculum_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", curriculum_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // curriculum_dto_list_envelope->error_code
    if(curriculum_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", curriculum_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // curriculum_dto_list_envelope->validation_details
    if(curriculum_dto_list_envelope->validation_details) {
    ...
    if (curriculum_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, curriculum_dto_list_envelope->validation_details) {
...
curriculum_dto_list_envelope_t *curriculum_dto_list_envelope_parseFromJSON(cJSON *curriculum_dto_list_envelopeJSON){
    ...
    // define the local map for curriculum_dto_list_envelope->validation_details
    ...
    // curriculum_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(curriculum_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // curriculum_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(curriculum_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // curriculum_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(curriculum_dto_list_envelopeJSON, "validationDetails");
