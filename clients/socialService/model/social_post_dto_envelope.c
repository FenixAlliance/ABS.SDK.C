...
social_post_dto_envelope_t *social_post_dto_envelope_create(
    ...
    social_post_dto_envelope_local_var->http_status = http_status;
    social_post_dto_envelope_local_var->error_code = error_code;
    social_post_dto_envelope_local_var->validation_details = validation_details;
...
void social_post_dto_envelope_free(social_post_dto_envelope_t *social_post_dto_envelope) {
    ...
    if (social_post_dto_envelope->error_code) {
        free(social_post_dto_envelope->error_code);
        social_post_dto_envelope->error_code = NULL;
    }
    if (social_post_dto_envelope->validation_details) {
        list_ForEach(listEntry, social_post_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(social_post_dto_envelope->validation_details);
        social_post_dto_envelope->validation_details = NULL;
    }
...
cJSON *social_post_dto_envelope_convertToJSON(social_post_dto_envelope_t *social_post_dto_envelope) {
    ...
    // social_post_dto_envelope->http_status
    if(social_post_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", social_post_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // social_post_dto_envelope->error_code
    if(social_post_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", social_post_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // social_post_dto_envelope->validation_details
    if(social_post_dto_envelope->validation_details) {
    ...
    if (social_post_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, social_post_dto_envelope->validation_details) {
...
social_post_dto_envelope_t *social_post_dto_envelope_parseFromJSON(cJSON *social_post_dto_envelopeJSON){
    ...
    // define the local map for social_post_dto_envelope->validation_details
    ...
    // social_post_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(social_post_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // social_post_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(social_post_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // social_post_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(social_post_dto_envelopeJSON, "validationDetails");
