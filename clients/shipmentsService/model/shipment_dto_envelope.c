...
shipment_dto_envelope_t *shipment_dto_envelope_create(
    ...
    shipment_dto_envelope_local_var->http_status = http_status;
    shipment_dto_envelope_local_var->error_code = error_code;
    shipment_dto_envelope_local_var->validation_details = validation_details;
...
void shipment_dto_envelope_free(shipment_dto_envelope_t *shipment_dto_envelope) {
    ...
    if (shipment_dto_envelope->error_code) {
        free(shipment_dto_envelope->error_code);
        shipment_dto_envelope->error_code = NULL;
    }
    if (shipment_dto_envelope->validation_details) {
        list_ForEach(listEntry, shipment_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(shipment_dto_envelope->validation_details);
        shipment_dto_envelope->validation_details = NULL;
    }
...
cJSON *shipment_dto_envelope_convertToJSON(shipment_dto_envelope_t *shipment_dto_envelope) {
    ...
    // shipment_dto_envelope->http_status
    if(shipment_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", shipment_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // shipment_dto_envelope->error_code
    if(shipment_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", shipment_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // shipment_dto_envelope->validation_details
    if(shipment_dto_envelope->validation_details) {
    ...
    if (shipment_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, shipment_dto_envelope->validation_details) {
...
shipment_dto_envelope_t *shipment_dto_envelope_parseFromJSON(cJSON *shipment_dto_envelopeJSON){
    ...
    // define the local map for shipment_dto_envelope->validation_details
    ...
    // shipment_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(shipment_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // shipment_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(shipment_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // shipment_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(shipment_dto_envelopeJSON, "validationDetails");
