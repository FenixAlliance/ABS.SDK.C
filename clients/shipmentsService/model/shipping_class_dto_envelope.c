...
shipping_class_dto_envelope_t *shipping_class_dto_envelope_create(
    ...
    shipping_class_dto_envelope_local_var->http_status = http_status;
    shipping_class_dto_envelope_local_var->error_code = error_code;
    shipping_class_dto_envelope_local_var->validation_details = validation_details;
...
void shipping_class_dto_envelope_free(shipping_class_dto_envelope_t *shipping_class_dto_envelope) {
    ...
    if (shipping_class_dto_envelope->error_code) {
        free(shipping_class_dto_envelope->error_code);
        shipping_class_dto_envelope->error_code = NULL;
    }
    if (shipping_class_dto_envelope->validation_details) {
        list_ForEach(listEntry, shipping_class_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(shipping_class_dto_envelope->validation_details);
        shipping_class_dto_envelope->validation_details = NULL;
    }
...
cJSON *shipping_class_dto_envelope_convertToJSON(shipping_class_dto_envelope_t *shipping_class_dto_envelope) {
    ...
    // shipping_class_dto_envelope->http_status
    if(shipping_class_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", shipping_class_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // shipping_class_dto_envelope->error_code
    if(shipping_class_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", shipping_class_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // shipping_class_dto_envelope->validation_details
    if(shipping_class_dto_envelope->validation_details) {
    ...
    if (shipping_class_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, shipping_class_dto_envelope->validation_details) {
...
shipping_class_dto_envelope_t *shipping_class_dto_envelope_parseFromJSON(cJSON *shipping_class_dto_envelopeJSON){
    ...
    // define the local map for shipping_class_dto_envelope->validation_details
    ...
    // shipping_class_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(shipping_class_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // shipping_class_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(shipping_class_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // shipping_class_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(shipping_class_dto_envelopeJSON, "validationDetails");
