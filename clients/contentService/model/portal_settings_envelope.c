...
portal_settings_envelope_t *portal_settings_envelope_create(
    ...
    portal_settings_envelope_local_var->http_status = http_status;
    portal_settings_envelope_local_var->error_code = error_code;
    portal_settings_envelope_local_var->validation_details = validation_details;
...
void portal_settings_envelope_free(portal_settings_envelope_t *portal_settings_envelope) {
    ...
    if (portal_settings_envelope->error_code) {
        free(portal_settings_envelope->error_code);
        portal_settings_envelope->error_code = NULL;
    }
    if (portal_settings_envelope->validation_details) {
        list_ForEach(listEntry, portal_settings_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(portal_settings_envelope->validation_details);
        portal_settings_envelope->validation_details = NULL;
    }
...
cJSON *portal_settings_envelope_convertToJSON(portal_settings_envelope_t *portal_settings_envelope) {
    ...
    // portal_settings_envelope->http_status
    if(portal_settings_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", portal_settings_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // portal_settings_envelope->error_code
    if(portal_settings_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", portal_settings_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // portal_settings_envelope->validation_details
    if(portal_settings_envelope->validation_details) {
    ...
    if (portal_settings_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, portal_settings_envelope->validation_details) {
...
portal_settings_envelope_t *portal_settings_envelope_parseFromJSON(cJSON *portal_settings_envelopeJSON){
    ...
    // define the local map for portal_settings_envelope->validation_details
    ...
    // portal_settings_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(portal_settings_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // portal_settings_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(portal_settings_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // portal_settings_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(portal_settings_envelopeJSON, "validationDetails");
