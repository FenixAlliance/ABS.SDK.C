...
web_portal_dto_t *web_portal_dto_create(
    ...
    if (!web_portal_dto_local_var) {
        return NULL;
    }
    web_portal_dto_local_var->id = id;
    web_portal_dto_local_var->timestamp = timestamp;
    web_portal_dto_local_var->root = root;
    web_portal_dto_local_var->title = title;
    web_portal_dto_local_var->domain = domain;
...
void web_portal_dto_free(web_portal_dto_t *web_portal_dto) {
    if(NULL == web_portal_dto){
        return ;
    }
    listEntry_t *listEntry;
    if (web_portal_dto->id) {
        free(web_portal_dto->id);
        web_portal_dto->id = NULL;
    }
    if (web_portal_dto->timestamp) {
        free(web_portal_dto->timestamp);
        web_portal_dto->timestamp = NULL;
    }
    if (web_portal_dto->title) {
        free(web_portal_dto->title);
        web_portal_dto->title = NULL;
    }
    if (web_portal_dto->domain) {
        free(web_portal_dto->domain);
        web_portal_dto->domain = NULL;
    }
    if (web_portal_dto->tenant_id) {
        free(web_portal_dto->tenant_id);
        web_portal_dto->tenant_id = NULL;
    }
    if (web_portal_dto->description) {
    ...
    if (web_portal_dto->business_portal_application_id) {
        ...
        web_portal_dto->business_portal_application_id = NULL;
    }
    free(web_portal_dto);
}
...
cJSON *web_portal_dto_convertToJSON(web_portal_dto_t *web_portal_dto) {
    ...
    // web_portal_dto->id
    if(web_portal_dto->id) {
    if(cJSON_AddStringToObject(item, "id", web_portal_dto->id) == NULL) {
    goto fail; //String
    }
    }


    // web_portal_dto->timestamp
    if(web_portal_dto->timestamp) {
    if(cJSON_AddStringToObject(item, "timestamp", web_portal_dto->timestamp) == NULL) {
    goto fail; //Date-Time
    }
    }
    ...
    // web_portal_dto->root
    if(web_portal_dto->root) {
    if(cJSON_AddBoolToObject(item, "root", web_portal_dto->root) == NULL) {
    goto fail; //Bool
    }
    }


    // web_portal_dto->title
    if(web_portal_dto->title) {
    if(cJSON_AddStringToObject(item, "title", web_portal_dto->title) == NULL) {
    goto fail; //String
    }
    }


    // web_portal_dto->domain
    if(web_portal_dto->domain) {
    if(cJSON_AddStringToObject(item, "domain", web_portal_dto->domain) == NULL) {
    goto fail; //String
    }
    }


    // web_portal_dto->disabled
    if(web_portal_dto->disabled) {
    if(cJSON_AddBoolToObject(item, "disabled", web_portal_dto->disabled) == NULL) {
    goto fail; //Bool
    }
    }


    // web_portal_dto->tenant_id
    if(web_portal_dto->tenant_id) {
    if(cJSON_AddStringToObject(item, "tenantId", web_portal_dto->tenant_id) == NULL) {
    goto fail; //String
    }
    }


    // web_portal_dto->description
    if(web_portal_dto->description) {
    if(cJSON_AddStringToObject(item, "description", web_portal_dto->description) == NULL) {
    goto fail; //String
    }
    }
...
web_portal_dto_t *web_portal_dto_parseFromJSON(cJSON *web_portal_dtoJSON){
    ...
    web_portal_dto_t *web_portal_dto_local_var = NULL;

    // web_portal_dto->id
    cJSON *id = cJSON_GetObjectItemCaseSensitive(web_portal_dtoJSON, "id");
    if (id) { 
    ...
    // web_portal_dto->timestamp
    cJSON *timestamp = cJSON_GetObjectItemCaseSensitive(web_portal_dtoJSON, "timestamp");
    if (timestamp) { 
    ...
    // web_portal_dto->root
    cJSON *root = cJSON_GetObjectItemCaseSensitive(web_portal_dtoJSON, "root");
    if (root) { 
    ...
    // web_portal_dto->title
    cJSON *title = cJSON_GetObjectItemCaseSensitive(web_portal_dtoJSON, "title");
    if (title) { 
    ...
    // web_portal_dto->domain
    cJSON *domain = cJSON_GetObjectItemCaseSensitive(web_portal_dtoJSON, "domain");
    if (domain) { 
    ...
    // web_portal_dto->disabled
    cJSON *disabled = cJSON_GetObjectItemCaseSensitive(web_portal_dtoJSON, "disabled");
    if (disabled) { 
    ...
    // web_portal_dto->tenant_id
    cJSON *tenant_id = cJSON_GetObjectItemCaseSensitive(web_portal_dtoJSON, "tenantId");
    if (tenant_id) { 
    ...
    // web_portal_dto->description
    cJSON *description = cJSON_GetObjectItemCaseSensitive(web_portal_dtoJSON, "description");
    if (description) { 
    ...
    web_portal_dto_local_var = web_portal_dto_create (
