...
blog_post_dto_list_envelope_t *blog_post_dto_list_envelope_create(
    ...
    blog_post_dto_list_envelope_local_var->http_status = http_status;
    blog_post_dto_list_envelope_local_var->error_code = error_code;
    blog_post_dto_list_envelope_local_var->validation_details = validation_details;
...
void blog_post_dto_list_envelope_free(blog_post_dto_list_envelope_t *blog_post_dto_list_envelope) {
    ...
    if (blog_post_dto_list_envelope->error_code) {
        free(blog_post_dto_list_envelope->error_code);
        blog_post_dto_list_envelope->error_code = NULL;
    }
    if (blog_post_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, blog_post_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(blog_post_dto_list_envelope->validation_details);
        blog_post_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *blog_post_dto_list_envelope_convertToJSON(blog_post_dto_list_envelope_t *blog_post_dto_list_envelope) {
    ...
    // blog_post_dto_list_envelope->http_status
    if(blog_post_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", blog_post_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // blog_post_dto_list_envelope->error_code
    if(blog_post_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", blog_post_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // blog_post_dto_list_envelope->validation_details
    if(blog_post_dto_list_envelope->validation_details) {
    ...
    if (blog_post_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, blog_post_dto_list_envelope->validation_details) {
...
blog_post_dto_list_envelope_t *blog_post_dto_list_envelope_parseFromJSON(cJSON *blog_post_dto_list_envelopeJSON){
    ...
    // define the local map for blog_post_dto_list_envelope->validation_details
    ...
    // blog_post_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(blog_post_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // blog_post_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(blog_post_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // blog_post_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(blog_post_dto_list_envelopeJSON, "validationDetails");
