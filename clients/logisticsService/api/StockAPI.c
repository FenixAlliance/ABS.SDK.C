...
StockAPI_getContactsAsync(apiClient_t *apiClient, char *tenantId, char *api_version, char *x_api_version)
{
    ...
    // create the path
    long sizeOfPath = strlen("/api/v2/LogisticsService/Stock")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/api/v2/LogisticsService/Stock");
    ...
    cJSON *StockAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    contact_dto_list_envelope_t *elementToReturn = contact_dto_list_envelope_parseFromJSON(StockAPIlocalVarJSON);
    cJSON_Delete(StockAPIlocalVarJSON);
