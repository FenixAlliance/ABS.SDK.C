...
vessel_dto_list_envelope_t *vessel_dto_list_envelope_create(
    ...
    vessel_dto_list_envelope_local_var->http_status = http_status;
    vessel_dto_list_envelope_local_var->error_code = error_code;
    vessel_dto_list_envelope_local_var->validation_details = validation_details;
...
void vessel_dto_list_envelope_free(vessel_dto_list_envelope_t *vessel_dto_list_envelope) {
    ...
    if (vessel_dto_list_envelope->error_code) {
        free(vessel_dto_list_envelope->error_code);
        vessel_dto_list_envelope->error_code = NULL;
    }
    if (vessel_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, vessel_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(vessel_dto_list_envelope->validation_details);
        vessel_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *vessel_dto_list_envelope_convertToJSON(vessel_dto_list_envelope_t *vessel_dto_list_envelope) {
    ...
    // vessel_dto_list_envelope->http_status
    if(vessel_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", vessel_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // vessel_dto_list_envelope->error_code
    if(vessel_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", vessel_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // vessel_dto_list_envelope->validation_details
    if(vessel_dto_list_envelope->validation_details) {
    ...
    if (vessel_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, vessel_dto_list_envelope->validation_details) {
...
vessel_dto_list_envelope_t *vessel_dto_list_envelope_parseFromJSON(cJSON *vessel_dto_list_envelopeJSON){
    ...
    // define the local map for vessel_dto_list_envelope->validation_details
    ...
    // vessel_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(vessel_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // vessel_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(vessel_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // vessel_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(vessel_dto_list_envelopeJSON, "validationDetails");
