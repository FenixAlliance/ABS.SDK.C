...
truck_driver_dto_list_envelope_t *truck_driver_dto_list_envelope_create(
    ...
    truck_driver_dto_list_envelope_local_var->http_status = http_status;
    truck_driver_dto_list_envelope_local_var->error_code = error_code;
    truck_driver_dto_list_envelope_local_var->validation_details = validation_details;
...
void truck_driver_dto_list_envelope_free(truck_driver_dto_list_envelope_t *truck_driver_dto_list_envelope) {
    ...
    if (truck_driver_dto_list_envelope->error_code) {
        free(truck_driver_dto_list_envelope->error_code);
        truck_driver_dto_list_envelope->error_code = NULL;
    }
    if (truck_driver_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, truck_driver_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(truck_driver_dto_list_envelope->validation_details);
        truck_driver_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *truck_driver_dto_list_envelope_convertToJSON(truck_driver_dto_list_envelope_t *truck_driver_dto_list_envelope) {
    ...
    // truck_driver_dto_list_envelope->http_status
    if(truck_driver_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", truck_driver_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // truck_driver_dto_list_envelope->error_code
    if(truck_driver_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", truck_driver_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // truck_driver_dto_list_envelope->validation_details
    if(truck_driver_dto_list_envelope->validation_details) {
    ...
    if (truck_driver_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, truck_driver_dto_list_envelope->validation_details) {
...
truck_driver_dto_list_envelope_t *truck_driver_dto_list_envelope_parseFromJSON(cJSON *truck_driver_dto_list_envelopeJSON){
    ...
    // define the local map for truck_driver_dto_list_envelope->validation_details
    ...
    // truck_driver_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(truck_driver_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // truck_driver_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(truck_driver_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // truck_driver_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(truck_driver_dto_list_envelopeJSON, "validationDetails");
