...
voyage_dto_envelope_t *voyage_dto_envelope_create(
    ...
    voyage_dto_envelope_local_var->http_status = http_status;
    voyage_dto_envelope_local_var->error_code = error_code;
    voyage_dto_envelope_local_var->validation_details = validation_details;
...
void voyage_dto_envelope_free(voyage_dto_envelope_t *voyage_dto_envelope) {
    ...
    if (voyage_dto_envelope->error_code) {
        free(voyage_dto_envelope->error_code);
        voyage_dto_envelope->error_code = NULL;
    }
    if (voyage_dto_envelope->validation_details) {
        list_ForEach(listEntry, voyage_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(voyage_dto_envelope->validation_details);
        voyage_dto_envelope->validation_details = NULL;
    }
...
cJSON *voyage_dto_envelope_convertToJSON(voyage_dto_envelope_t *voyage_dto_envelope) {
    ...
    // voyage_dto_envelope->http_status
    if(voyage_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", voyage_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // voyage_dto_envelope->error_code
    if(voyage_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", voyage_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // voyage_dto_envelope->validation_details
    if(voyage_dto_envelope->validation_details) {
    ...
    if (voyage_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, voyage_dto_envelope->validation_details) {
...
voyage_dto_envelope_t *voyage_dto_envelope_parseFromJSON(cJSON *voyage_dto_envelopeJSON){
    ...
    // define the local map for voyage_dto_envelope->validation_details
    ...
    // voyage_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(voyage_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // voyage_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(voyage_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // voyage_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(voyage_dto_envelopeJSON, "validationDetails");
