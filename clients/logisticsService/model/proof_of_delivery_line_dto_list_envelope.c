...
proof_of_delivery_line_dto_list_envelope_t *proof_of_delivery_line_dto_list_envelope_create(
    ...
    proof_of_delivery_line_dto_list_envelope_local_var->http_status = http_status;
    proof_of_delivery_line_dto_list_envelope_local_var->error_code = error_code;
    proof_of_delivery_line_dto_list_envelope_local_var->validation_details = validation_details;
...
void proof_of_delivery_line_dto_list_envelope_free(proof_of_delivery_line_dto_list_envelope_t *proof_of_delivery_line_dto_list_envelope) {
    ...
    if (proof_of_delivery_line_dto_list_envelope->error_code) {
        free(proof_of_delivery_line_dto_list_envelope->error_code);
        proof_of_delivery_line_dto_list_envelope->error_code = NULL;
    }
    if (proof_of_delivery_line_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, proof_of_delivery_line_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(proof_of_delivery_line_dto_list_envelope->validation_details);
        proof_of_delivery_line_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *proof_of_delivery_line_dto_list_envelope_convertToJSON(proof_of_delivery_line_dto_list_envelope_t *proof_of_delivery_line_dto_list_envelope) {
    ...
    // proof_of_delivery_line_dto_list_envelope->http_status
    if(proof_of_delivery_line_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", proof_of_delivery_line_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // proof_of_delivery_line_dto_list_envelope->error_code
    if(proof_of_delivery_line_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", proof_of_delivery_line_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // proof_of_delivery_line_dto_list_envelope->validation_details
    if(proof_of_delivery_line_dto_list_envelope->validation_details) {
    ...
    if (proof_of_delivery_line_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, proof_of_delivery_line_dto_list_envelope->validation_details) {
...
proof_of_delivery_line_dto_list_envelope_t *proof_of_delivery_line_dto_list_envelope_parseFromJSON(cJSON *proof_of_delivery_line_dto_list_envelopeJSON){
    ...
    // define the local map for proof_of_delivery_line_dto_list_envelope->validation_details
    ...
    // proof_of_delivery_line_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(proof_of_delivery_line_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // proof_of_delivery_line_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(proof_of_delivery_line_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // proof_of_delivery_line_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(proof_of_delivery_line_dto_list_envelopeJSON, "validationDetails");
