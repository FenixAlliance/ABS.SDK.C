...
airway_bill_dto_envelope_t *airway_bill_dto_envelope_create(
    ...
    airway_bill_dto_envelope_local_var->http_status = http_status;
    airway_bill_dto_envelope_local_var->error_code = error_code;
    airway_bill_dto_envelope_local_var->validation_details = validation_details;
...
void airway_bill_dto_envelope_free(airway_bill_dto_envelope_t *airway_bill_dto_envelope) {
    ...
    if (airway_bill_dto_envelope->error_code) {
        free(airway_bill_dto_envelope->error_code);
        airway_bill_dto_envelope->error_code = NULL;
    }
    if (airway_bill_dto_envelope->validation_details) {
        list_ForEach(listEntry, airway_bill_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(airway_bill_dto_envelope->validation_details);
        airway_bill_dto_envelope->validation_details = NULL;
    }
...
cJSON *airway_bill_dto_envelope_convertToJSON(airway_bill_dto_envelope_t *airway_bill_dto_envelope) {
    ...
    // airway_bill_dto_envelope->http_status
    if(airway_bill_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", airway_bill_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // airway_bill_dto_envelope->error_code
    if(airway_bill_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", airway_bill_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // airway_bill_dto_envelope->validation_details
    if(airway_bill_dto_envelope->validation_details) {
    ...
    if (airway_bill_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, airway_bill_dto_envelope->validation_details) {
...
airway_bill_dto_envelope_t *airway_bill_dto_envelope_parseFromJSON(cJSON *airway_bill_dto_envelopeJSON){
    ...
    // define the local map for airway_bill_dto_envelope->validation_details
    ...
    // airway_bill_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(airway_bill_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // airway_bill_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(airway_bill_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // airway_bill_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(airway_bill_dto_envelopeJSON, "validationDetails");
