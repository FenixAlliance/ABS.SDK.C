...
user_dto_envelope_t *user_dto_envelope_create(
    ...
    user_dto_envelope_local_var->http_status = http_status;
    user_dto_envelope_local_var->error_code = error_code;
    user_dto_envelope_local_var->validation_details = validation_details;
...
void user_dto_envelope_free(user_dto_envelope_t *user_dto_envelope) {
    ...
    if (user_dto_envelope->error_code) {
        free(user_dto_envelope->error_code);
        user_dto_envelope->error_code = NULL;
    }
    if (user_dto_envelope->validation_details) {
        list_ForEach(listEntry, user_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(user_dto_envelope->validation_details);
        user_dto_envelope->validation_details = NULL;
    }
...
cJSON *user_dto_envelope_convertToJSON(user_dto_envelope_t *user_dto_envelope) {
    ...
    // user_dto_envelope->http_status
    if(user_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", user_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // user_dto_envelope->error_code
    if(user_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", user_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // user_dto_envelope->validation_details
    if(user_dto_envelope->validation_details) {
    ...
    if (user_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, user_dto_envelope->validation_details) {
...
user_dto_envelope_t *user_dto_envelope_parseFromJSON(cJSON *user_dto_envelopeJSON){
    ...
    // define the local map for user_dto_envelope->validation_details
    ...
    // user_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(user_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // user_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(user_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // user_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(user_dto_envelopeJSON, "validationDetails");
