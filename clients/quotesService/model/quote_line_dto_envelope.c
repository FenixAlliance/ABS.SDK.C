...
quote_line_dto_envelope_t *quote_line_dto_envelope_create(
    ...
    quote_line_dto_envelope_local_var->http_status = http_status;
    quote_line_dto_envelope_local_var->error_code = error_code;
    quote_line_dto_envelope_local_var->validation_details = validation_details;
...
void quote_line_dto_envelope_free(quote_line_dto_envelope_t *quote_line_dto_envelope) {
    ...
    if (quote_line_dto_envelope->error_code) {
        free(quote_line_dto_envelope->error_code);
        quote_line_dto_envelope->error_code = NULL;
    }
    if (quote_line_dto_envelope->validation_details) {
        list_ForEach(listEntry, quote_line_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(quote_line_dto_envelope->validation_details);
        quote_line_dto_envelope->validation_details = NULL;
    }
...
cJSON *quote_line_dto_envelope_convertToJSON(quote_line_dto_envelope_t *quote_line_dto_envelope) {
    ...
    // quote_line_dto_envelope->http_status
    if(quote_line_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", quote_line_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // quote_line_dto_envelope->error_code
    if(quote_line_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", quote_line_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // quote_line_dto_envelope->validation_details
    if(quote_line_dto_envelope->validation_details) {
    ...
    if (quote_line_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, quote_line_dto_envelope->validation_details) {
...
quote_line_dto_envelope_t *quote_line_dto_envelope_parseFromJSON(cJSON *quote_line_dto_envelopeJSON){
    ...
    // define the local map for quote_line_dto_envelope->validation_details
    ...
    // quote_line_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(quote_line_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // quote_line_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(quote_line_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // quote_line_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(quote_line_dto_envelopeJSON, "validationDetails");
