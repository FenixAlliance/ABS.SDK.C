...
InventoryAPI_getInventoryDetailsAsync(apiClient_t *apiClient, char *stockItemId, char *api_version, char *x_api_version)
{
    ...
    // create the path
    long sizeOfPath = strlen("/api/v2/InventoryService/Inventory/{stockItemId}/Details")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/api/v2/InventoryService/Inventory/{stockItemId}/Details");


    // Path Params
    long sizeOfPathParams_stockItemId = strlen(stockItemId)+3 + strlen("{ stockItemId }");
    if(stockItemId == NULL) {
        goto end;
    }
    char* localVarToReplace_stockItemId = malloc(sizeOfPathParams_stockItemId);
    sprintf(localVarToReplace_stockItemId, "{%s}", "stockItemId");

    localVarPath = strReplace(localVarPath, localVarToReplace_stockItemId, stockItemId);
    if(stockItemId == NULL) {
        goto end;
    }
    char* localVarToReplace_stockItemId = malloc(sizeOfPathParams_stockItemId);
    ...
    free(localVarPath);
    free(localVarToReplace_stockItemId);
