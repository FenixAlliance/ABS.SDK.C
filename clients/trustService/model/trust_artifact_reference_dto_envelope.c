...
trust_artifact_reference_dto_envelope_t *trust_artifact_reference_dto_envelope_create(
    ...
    trust_artifact_reference_dto_envelope_local_var->http_status = http_status;
    trust_artifact_reference_dto_envelope_local_var->error_code = error_code;
    trust_artifact_reference_dto_envelope_local_var->validation_details = validation_details;
...
void trust_artifact_reference_dto_envelope_free(trust_artifact_reference_dto_envelope_t *trust_artifact_reference_dto_envelope) {
    ...
    if (trust_artifact_reference_dto_envelope->error_code) {
        free(trust_artifact_reference_dto_envelope->error_code);
        trust_artifact_reference_dto_envelope->error_code = NULL;
    }
    if (trust_artifact_reference_dto_envelope->validation_details) {
        list_ForEach(listEntry, trust_artifact_reference_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(trust_artifact_reference_dto_envelope->validation_details);
        trust_artifact_reference_dto_envelope->validation_details = NULL;
    }
...
cJSON *trust_artifact_reference_dto_envelope_convertToJSON(trust_artifact_reference_dto_envelope_t *trust_artifact_reference_dto_envelope) {
    ...
    // trust_artifact_reference_dto_envelope->http_status
    if(trust_artifact_reference_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", trust_artifact_reference_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // trust_artifact_reference_dto_envelope->error_code
    if(trust_artifact_reference_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", trust_artifact_reference_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // trust_artifact_reference_dto_envelope->validation_details
    if(trust_artifact_reference_dto_envelope->validation_details) {
    ...
    if (trust_artifact_reference_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, trust_artifact_reference_dto_envelope->validation_details) {
...
trust_artifact_reference_dto_envelope_t *trust_artifact_reference_dto_envelope_parseFromJSON(cJSON *trust_artifact_reference_dto_envelopeJSON){
    ...
    // define the local map for trust_artifact_reference_dto_envelope->validation_details
    ...
    // trust_artifact_reference_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(trust_artifact_reference_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // trust_artifact_reference_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(trust_artifact_reference_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // trust_artifact_reference_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(trust_artifact_reference_dto_envelopeJSON, "validationDetails");
