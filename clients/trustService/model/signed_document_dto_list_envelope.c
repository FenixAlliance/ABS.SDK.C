...
signed_document_dto_list_envelope_t *signed_document_dto_list_envelope_create(
    ...
    signed_document_dto_list_envelope_local_var->http_status = http_status;
    signed_document_dto_list_envelope_local_var->error_code = error_code;
    signed_document_dto_list_envelope_local_var->validation_details = validation_details;
...
void signed_document_dto_list_envelope_free(signed_document_dto_list_envelope_t *signed_document_dto_list_envelope) {
    ...
    if (signed_document_dto_list_envelope->error_code) {
        free(signed_document_dto_list_envelope->error_code);
        signed_document_dto_list_envelope->error_code = NULL;
    }
    if (signed_document_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, signed_document_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(signed_document_dto_list_envelope->validation_details);
        signed_document_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *signed_document_dto_list_envelope_convertToJSON(signed_document_dto_list_envelope_t *signed_document_dto_list_envelope) {
    ...
    // signed_document_dto_list_envelope->http_status
    if(signed_document_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", signed_document_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // signed_document_dto_list_envelope->error_code
    if(signed_document_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", signed_document_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // signed_document_dto_list_envelope->validation_details
    if(signed_document_dto_list_envelope->validation_details) {
    ...
    if (signed_document_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, signed_document_dto_list_envelope->validation_details) {
...
signed_document_dto_list_envelope_t *signed_document_dto_list_envelope_parseFromJSON(cJSON *signed_document_dto_list_envelopeJSON){
    ...
    // define the local map for signed_document_dto_list_envelope->validation_details
    ...
    // signed_document_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(signed_document_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // signed_document_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(signed_document_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // signed_document_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(signed_document_dto_list_envelopeJSON, "validationDetails");
