...
trust_signing_readiness_dto_envelope_t *trust_signing_readiness_dto_envelope_create(
    ...
    trust_signing_readiness_dto_envelope_local_var->http_status = http_status;
    trust_signing_readiness_dto_envelope_local_var->error_code = error_code;
    trust_signing_readiness_dto_envelope_local_var->validation_details = validation_details;
...
void trust_signing_readiness_dto_envelope_free(trust_signing_readiness_dto_envelope_t *trust_signing_readiness_dto_envelope) {
    ...
    if (trust_signing_readiness_dto_envelope->error_code) {
        free(trust_signing_readiness_dto_envelope->error_code);
        trust_signing_readiness_dto_envelope->error_code = NULL;
    }
    if (trust_signing_readiness_dto_envelope->validation_details) {
        list_ForEach(listEntry, trust_signing_readiness_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(trust_signing_readiness_dto_envelope->validation_details);
        trust_signing_readiness_dto_envelope->validation_details = NULL;
    }
...
cJSON *trust_signing_readiness_dto_envelope_convertToJSON(trust_signing_readiness_dto_envelope_t *trust_signing_readiness_dto_envelope) {
    ...
    // trust_signing_readiness_dto_envelope->http_status
    if(trust_signing_readiness_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", trust_signing_readiness_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // trust_signing_readiness_dto_envelope->error_code
    if(trust_signing_readiness_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", trust_signing_readiness_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // trust_signing_readiness_dto_envelope->validation_details
    if(trust_signing_readiness_dto_envelope->validation_details) {
    ...
    if (trust_signing_readiness_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, trust_signing_readiness_dto_envelope->validation_details) {
...
trust_signing_readiness_dto_envelope_t *trust_signing_readiness_dto_envelope_parseFromJSON(cJSON *trust_signing_readiness_dto_envelopeJSON){
    ...
    // define the local map for trust_signing_readiness_dto_envelope->validation_details
    ...
    // trust_signing_readiness_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(trust_signing_readiness_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // trust_signing_readiness_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(trust_signing_readiness_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // trust_signing_readiness_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(trust_signing_readiness_dto_envelopeJSON, "validationDetails");
