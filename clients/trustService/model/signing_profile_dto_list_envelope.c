...
signing_profile_dto_list_envelope_t *signing_profile_dto_list_envelope_create(
    ...
    signing_profile_dto_list_envelope_local_var->http_status = http_status;
    signing_profile_dto_list_envelope_local_var->error_code = error_code;
    signing_profile_dto_list_envelope_local_var->validation_details = validation_details;
...
void signing_profile_dto_list_envelope_free(signing_profile_dto_list_envelope_t *signing_profile_dto_list_envelope) {
    ...
    if (signing_profile_dto_list_envelope->error_code) {
        free(signing_profile_dto_list_envelope->error_code);
        signing_profile_dto_list_envelope->error_code = NULL;
    }
    if (signing_profile_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, signing_profile_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(signing_profile_dto_list_envelope->validation_details);
        signing_profile_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *signing_profile_dto_list_envelope_convertToJSON(signing_profile_dto_list_envelope_t *signing_profile_dto_list_envelope) {
    ...
    // signing_profile_dto_list_envelope->http_status
    if(signing_profile_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", signing_profile_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // signing_profile_dto_list_envelope->error_code
    if(signing_profile_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", signing_profile_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // signing_profile_dto_list_envelope->validation_details
    if(signing_profile_dto_list_envelope->validation_details) {
    ...
    if (signing_profile_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, signing_profile_dto_list_envelope->validation_details) {
...
signing_profile_dto_list_envelope_t *signing_profile_dto_list_envelope_parseFromJSON(cJSON *signing_profile_dto_list_envelopeJSON){
    ...
    // define the local map for signing_profile_dto_list_envelope->validation_details
    ...
    // signing_profile_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(signing_profile_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // signing_profile_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(signing_profile_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // signing_profile_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(signing_profile_dto_list_envelopeJSON, "validationDetails");
