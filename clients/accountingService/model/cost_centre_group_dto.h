/*
 * cost_centre_group_dto.h
 *
 * 
 */

#ifndef _cost_centre_group_dto_H_
#define _cost_centre_group_dto_H_

#include <string.h>
#include "../external/cJSON.h"
#include "../include/list.h"
#include "../include/keyValuePair.h"
#include "../include/binary.h"

typedef struct cost_centre_group_dto_t cost_centre_group_dto_t;




typedef struct cost_centre_group_dto_t {
    char *id; // string
    char *timestamp; //date time
    char *name; // string
    char *description; // string
    int disabled; //boolean
    char *tenant_id; // string
    char *parent_cost_centres_group_id; // string

} cost_centre_group_dto_t;

cost_centre_group_dto_t *cost_centre_group_dto_create(
    char *id,
    char *timestamp,
    char *name,
    char *description,
    int disabled,
    char *tenant_id,
    char *parent_cost_centres_group_id
);

void cost_centre_group_dto_free(cost_centre_group_dto_t *cost_centre_group_dto);

cost_centre_group_dto_t *cost_centre_group_dto_parseFromJSON(cJSON *cost_centre_group_dtoJSON);

cJSON *cost_centre_group_dto_convertToJSON(cost_centre_group_dto_t *cost_centre_group_dto);

#endif /* _cost_centre_group_dto_H_ */

