...
ledger_type_dto_envelope_t *ledger_type_dto_envelope_create(
    ...
    ledger_type_dto_envelope_local_var->http_status = http_status;
    ledger_type_dto_envelope_local_var->error_code = error_code;
    ledger_type_dto_envelope_local_var->validation_details = validation_details;
...
void ledger_type_dto_envelope_free(ledger_type_dto_envelope_t *ledger_type_dto_envelope) {
    ...
    if (ledger_type_dto_envelope->error_code) {
        free(ledger_type_dto_envelope->error_code);
        ledger_type_dto_envelope->error_code = NULL;
    }
    if (ledger_type_dto_envelope->validation_details) {
        list_ForEach(listEntry, ledger_type_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(ledger_type_dto_envelope->validation_details);
        ledger_type_dto_envelope->validation_details = NULL;
    }
...
cJSON *ledger_type_dto_envelope_convertToJSON(ledger_type_dto_envelope_t *ledger_type_dto_envelope) {
    ...
    // ledger_type_dto_envelope->http_status
    if(ledger_type_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", ledger_type_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // ledger_type_dto_envelope->error_code
    if(ledger_type_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", ledger_type_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // ledger_type_dto_envelope->validation_details
    if(ledger_type_dto_envelope->validation_details) {
    ...
    if (ledger_type_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, ledger_type_dto_envelope->validation_details) {
...
ledger_type_dto_envelope_t *ledger_type_dto_envelope_parseFromJSON(cJSON *ledger_type_dto_envelopeJSON){
    ...
    // define the local map for ledger_type_dto_envelope->validation_details
    ...
    // ledger_type_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(ledger_type_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // ledger_type_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(ledger_type_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // ledger_type_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(ledger_type_dto_envelopeJSON, "validationDetails");
