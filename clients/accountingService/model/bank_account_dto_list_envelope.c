...
bank_account_dto_list_envelope_t *bank_account_dto_list_envelope_create(
    ...
    bank_account_dto_list_envelope_local_var->http_status = http_status;
    bank_account_dto_list_envelope_local_var->error_code = error_code;
    bank_account_dto_list_envelope_local_var->validation_details = validation_details;
...
void bank_account_dto_list_envelope_free(bank_account_dto_list_envelope_t *bank_account_dto_list_envelope) {
    ...
    if (bank_account_dto_list_envelope->error_code) {
        free(bank_account_dto_list_envelope->error_code);
        bank_account_dto_list_envelope->error_code = NULL;
    }
    if (bank_account_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, bank_account_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(bank_account_dto_list_envelope->validation_details);
        bank_account_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *bank_account_dto_list_envelope_convertToJSON(bank_account_dto_list_envelope_t *bank_account_dto_list_envelope) {
    ...
    // bank_account_dto_list_envelope->http_status
    if(bank_account_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", bank_account_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // bank_account_dto_list_envelope->error_code
    if(bank_account_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", bank_account_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // bank_account_dto_list_envelope->validation_details
    if(bank_account_dto_list_envelope->validation_details) {
    ...
    if (bank_account_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, bank_account_dto_list_envelope->validation_details) {
...
bank_account_dto_list_envelope_t *bank_account_dto_list_envelope_parseFromJSON(cJSON *bank_account_dto_list_envelopeJSON){
    ...
    // define the local map for bank_account_dto_list_envelope->validation_details
    ...
    // bank_account_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(bank_account_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // bank_account_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(bank_account_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // bank_account_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(bank_account_dto_list_envelopeJSON, "validationDetails");
