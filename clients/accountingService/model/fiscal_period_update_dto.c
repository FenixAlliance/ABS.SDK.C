...
fiscal_period_update_dto_t *fiscal_period_update_dto_create(
    ...
    fiscal_period_update_dto_local_var->tenant_id = tenant_id;
    fiscal_period_update_dto_local_var->enrollment_id = enrollment_id;
...
void fiscal_period_update_dto_free(fiscal_period_update_dto_t *fiscal_period_update_dto) {
    ...
    if (fiscal_period_update_dto->tenant_id) {
        free(fiscal_period_update_dto->tenant_id);
        fiscal_period_update_dto->tenant_id = NULL;
    }
    if (fiscal_period_update_dto->enrollment_id) {
        free(fiscal_period_update_dto->enrollment_id);
        fiscal_period_update_dto->enrollment_id = NULL;
    }
...
cJSON *fiscal_period_update_dto_convertToJSON(fiscal_period_update_dto_t *fiscal_period_update_dto) {
    ...
    // fiscal_period_update_dto->tenant_id
    if(fiscal_period_update_dto->tenant_id) {
    if(cJSON_AddStringToObject(item, "tenantId", fiscal_period_update_dto->tenant_id) == NULL) {
    goto fail; //String
    }
    }


    // fiscal_period_update_dto->enrollment_id
    if(fiscal_period_update_dto->enrollment_id) {
    if(cJSON_AddStringToObject(item, "enrollmentId", fiscal_period_update_dto->enrollment_id) == NULL) {
    goto fail; //String
    }
    }
...
fiscal_period_update_dto_t *fiscal_period_update_dto_parseFromJSON(cJSON *fiscal_period_update_dtoJSON){
    ...
    // fiscal_period_update_dto->tenant_id
    cJSON *tenant_id = cJSON_GetObjectItemCaseSensitive(fiscal_period_update_dtoJSON, "tenantId");
    if (tenant_id) { 
    ...
    // fiscal_period_update_dto->enrollment_id
    cJSON *enrollment_id = cJSON_GetObjectItemCaseSensitive(fiscal_period_update_dtoJSON, "enrollmentId");
    if (enrollment_id) { 
