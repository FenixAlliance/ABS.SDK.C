...
fiscal_regime_dto_list_envelope_t *fiscal_regime_dto_list_envelope_create(
    ...
    fiscal_regime_dto_list_envelope_local_var->http_status = http_status;
    fiscal_regime_dto_list_envelope_local_var->error_code = error_code;
    fiscal_regime_dto_list_envelope_local_var->validation_details = validation_details;
...
void fiscal_regime_dto_list_envelope_free(fiscal_regime_dto_list_envelope_t *fiscal_regime_dto_list_envelope) {
    ...
    if (fiscal_regime_dto_list_envelope->error_code) {
        free(fiscal_regime_dto_list_envelope->error_code);
        fiscal_regime_dto_list_envelope->error_code = NULL;
    }
    if (fiscal_regime_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, fiscal_regime_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(fiscal_regime_dto_list_envelope->validation_details);
        fiscal_regime_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *fiscal_regime_dto_list_envelope_convertToJSON(fiscal_regime_dto_list_envelope_t *fiscal_regime_dto_list_envelope) {
    ...
    // fiscal_regime_dto_list_envelope->http_status
    if(fiscal_regime_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", fiscal_regime_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // fiscal_regime_dto_list_envelope->error_code
    if(fiscal_regime_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", fiscal_regime_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // fiscal_regime_dto_list_envelope->validation_details
    if(fiscal_regime_dto_list_envelope->validation_details) {
    ...
    if (fiscal_regime_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, fiscal_regime_dto_list_envelope->validation_details) {
...
fiscal_regime_dto_list_envelope_t *fiscal_regime_dto_list_envelope_parseFromJSON(cJSON *fiscal_regime_dto_list_envelopeJSON){
    ...
    // define the local map for fiscal_regime_dto_list_envelope->validation_details
    ...
    // fiscal_regime_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(fiscal_regime_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // fiscal_regime_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(fiscal_regime_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // fiscal_regime_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(fiscal_regime_dto_list_envelopeJSON, "validationDetails");
