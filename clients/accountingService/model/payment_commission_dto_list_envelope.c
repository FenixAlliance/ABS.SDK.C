...
payment_commission_dto_list_envelope_t *payment_commission_dto_list_envelope_create(
    ...
    payment_commission_dto_list_envelope_local_var->http_status = http_status;
    payment_commission_dto_list_envelope_local_var->error_code = error_code;
    payment_commission_dto_list_envelope_local_var->validation_details = validation_details;
...
void payment_commission_dto_list_envelope_free(payment_commission_dto_list_envelope_t *payment_commission_dto_list_envelope) {
    ...
    if (payment_commission_dto_list_envelope->error_code) {
        free(payment_commission_dto_list_envelope->error_code);
        payment_commission_dto_list_envelope->error_code = NULL;
    }
    if (payment_commission_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, payment_commission_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(payment_commission_dto_list_envelope->validation_details);
        payment_commission_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *payment_commission_dto_list_envelope_convertToJSON(payment_commission_dto_list_envelope_t *payment_commission_dto_list_envelope) {
    ...
    // payment_commission_dto_list_envelope->http_status
    if(payment_commission_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", payment_commission_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // payment_commission_dto_list_envelope->error_code
    if(payment_commission_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", payment_commission_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // payment_commission_dto_list_envelope->validation_details
    if(payment_commission_dto_list_envelope->validation_details) {
    ...
    if (payment_commission_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, payment_commission_dto_list_envelope->validation_details) {
...
payment_commission_dto_list_envelope_t *payment_commission_dto_list_envelope_parseFromJSON(cJSON *payment_commission_dto_list_envelopeJSON){
    ...
    // define the local map for payment_commission_dto_list_envelope->validation_details
    ...
    // payment_commission_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(payment_commission_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // payment_commission_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(payment_commission_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // payment_commission_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(payment_commission_dto_list_envelopeJSON, "validationDetails");
