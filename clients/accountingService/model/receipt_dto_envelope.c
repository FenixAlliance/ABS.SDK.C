...
receipt_dto_envelope_t *receipt_dto_envelope_create(
    ...
    receipt_dto_envelope_local_var->http_status = http_status;
    receipt_dto_envelope_local_var->error_code = error_code;
    receipt_dto_envelope_local_var->validation_details = validation_details;
...
void receipt_dto_envelope_free(receipt_dto_envelope_t *receipt_dto_envelope) {
    ...
    if (receipt_dto_envelope->error_code) {
        free(receipt_dto_envelope->error_code);
        receipt_dto_envelope->error_code = NULL;
    }
    if (receipt_dto_envelope->validation_details) {
        list_ForEach(listEntry, receipt_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(receipt_dto_envelope->validation_details);
        receipt_dto_envelope->validation_details = NULL;
    }
...
cJSON *receipt_dto_envelope_convertToJSON(receipt_dto_envelope_t *receipt_dto_envelope) {
    ...
    // receipt_dto_envelope->http_status
    if(receipt_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", receipt_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // receipt_dto_envelope->error_code
    if(receipt_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", receipt_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // receipt_dto_envelope->validation_details
    if(receipt_dto_envelope->validation_details) {
    ...
    if (receipt_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, receipt_dto_envelope->validation_details) {
...
receipt_dto_envelope_t *receipt_dto_envelope_parseFromJSON(cJSON *receipt_dto_envelopeJSON){
    ...
    // define the local map for receipt_dto_envelope->validation_details
    ...
    // receipt_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(receipt_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // receipt_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(receipt_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // receipt_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(receipt_dto_envelopeJSON, "validationDetails");
