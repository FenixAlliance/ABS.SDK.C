...
accounting_period_dto_list_envelope_t *accounting_period_dto_list_envelope_create(
    ...
    accounting_period_dto_list_envelope_local_var->http_status = http_status;
    accounting_period_dto_list_envelope_local_var->error_code = error_code;
    accounting_period_dto_list_envelope_local_var->validation_details = validation_details;
...
void accounting_period_dto_list_envelope_free(accounting_period_dto_list_envelope_t *accounting_period_dto_list_envelope) {
    ...
    if (accounting_period_dto_list_envelope->error_code) {
        free(accounting_period_dto_list_envelope->error_code);
        accounting_period_dto_list_envelope->error_code = NULL;
    }
    if (accounting_period_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, accounting_period_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(accounting_period_dto_list_envelope->validation_details);
        accounting_period_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *accounting_period_dto_list_envelope_convertToJSON(accounting_period_dto_list_envelope_t *accounting_period_dto_list_envelope) {
    ...
    // accounting_period_dto_list_envelope->http_status
    if(accounting_period_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", accounting_period_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // accounting_period_dto_list_envelope->error_code
    if(accounting_period_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", accounting_period_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // accounting_period_dto_list_envelope->validation_details
    if(accounting_period_dto_list_envelope->validation_details) {
    ...
    if (accounting_period_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, accounting_period_dto_list_envelope->validation_details) {
...
accounting_period_dto_list_envelope_t *accounting_period_dto_list_envelope_parseFromJSON(cJSON *accounting_period_dto_list_envelopeJSON){
    ...
    // define the local map for accounting_period_dto_list_envelope->validation_details
    ...
    // accounting_period_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(accounting_period_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // accounting_period_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(accounting_period_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // accounting_period_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(accounting_period_dto_list_envelopeJSON, "validationDetails");
