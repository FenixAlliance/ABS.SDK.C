...
share_issuance_dto_envelope_t *share_issuance_dto_envelope_create(
    ...
    share_issuance_dto_envelope_local_var->http_status = http_status;
    share_issuance_dto_envelope_local_var->error_code = error_code;
    share_issuance_dto_envelope_local_var->validation_details = validation_details;
...
void share_issuance_dto_envelope_free(share_issuance_dto_envelope_t *share_issuance_dto_envelope) {
    ...
    if (share_issuance_dto_envelope->error_code) {
        free(share_issuance_dto_envelope->error_code);
        share_issuance_dto_envelope->error_code = NULL;
    }
    if (share_issuance_dto_envelope->validation_details) {
        list_ForEach(listEntry, share_issuance_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(share_issuance_dto_envelope->validation_details);
        share_issuance_dto_envelope->validation_details = NULL;
    }
...
cJSON *share_issuance_dto_envelope_convertToJSON(share_issuance_dto_envelope_t *share_issuance_dto_envelope) {
    ...
    // share_issuance_dto_envelope->http_status
    if(share_issuance_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", share_issuance_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // share_issuance_dto_envelope->error_code
    if(share_issuance_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", share_issuance_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // share_issuance_dto_envelope->validation_details
    if(share_issuance_dto_envelope->validation_details) {
    ...
    if (share_issuance_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, share_issuance_dto_envelope->validation_details) {
...
share_issuance_dto_envelope_t *share_issuance_dto_envelope_parseFromJSON(cJSON *share_issuance_dto_envelopeJSON){
    ...
    // define the local map for share_issuance_dto_envelope->validation_details
    ...
    // share_issuance_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(share_issuance_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // share_issuance_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(share_issuance_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // share_issuance_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(share_issuance_dto_envelopeJSON, "validationDetails");
