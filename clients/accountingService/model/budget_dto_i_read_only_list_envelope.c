...
budget_dto_i_read_only_list_envelope_t *budget_dto_i_read_only_list_envelope_create(
    ...
    budget_dto_i_read_only_list_envelope_local_var->http_status = http_status;
    budget_dto_i_read_only_list_envelope_local_var->error_code = error_code;
    budget_dto_i_read_only_list_envelope_local_var->validation_details = validation_details;
...
void budget_dto_i_read_only_list_envelope_free(budget_dto_i_read_only_list_envelope_t *budget_dto_i_read_only_list_envelope) {
    ...
    if (budget_dto_i_read_only_list_envelope->error_code) {
        free(budget_dto_i_read_only_list_envelope->error_code);
        budget_dto_i_read_only_list_envelope->error_code = NULL;
    }
    if (budget_dto_i_read_only_list_envelope->validation_details) {
        list_ForEach(listEntry, budget_dto_i_read_only_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(budget_dto_i_read_only_list_envelope->validation_details);
        budget_dto_i_read_only_list_envelope->validation_details = NULL;
    }
...
cJSON *budget_dto_i_read_only_list_envelope_convertToJSON(budget_dto_i_read_only_list_envelope_t *budget_dto_i_read_only_list_envelope) {
    ...
    // budget_dto_i_read_only_list_envelope->http_status
    if(budget_dto_i_read_only_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", budget_dto_i_read_only_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // budget_dto_i_read_only_list_envelope->error_code
    if(budget_dto_i_read_only_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", budget_dto_i_read_only_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // budget_dto_i_read_only_list_envelope->validation_details
    if(budget_dto_i_read_only_list_envelope->validation_details) {
    ...
    if (budget_dto_i_read_only_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, budget_dto_i_read_only_list_envelope->validation_details) {
...
budget_dto_i_read_only_list_envelope_t *budget_dto_i_read_only_list_envelope_parseFromJSON(cJSON *budget_dto_i_read_only_list_envelopeJSON){
    ...
    // define the local map for budget_dto_i_read_only_list_envelope->validation_details
    ...
    // budget_dto_i_read_only_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(budget_dto_i_read_only_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // budget_dto_i_read_only_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(budget_dto_i_read_only_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // budget_dto_i_read_only_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(budget_dto_i_read_only_list_envelopeJSON, "validationDetails");
