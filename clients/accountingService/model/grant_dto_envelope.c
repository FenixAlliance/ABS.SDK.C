...
grant_dto_envelope_t *grant_dto_envelope_create(
    ...
    grant_dto_envelope_local_var->http_status = http_status;
    grant_dto_envelope_local_var->error_code = error_code;
    grant_dto_envelope_local_var->validation_details = validation_details;
...
void grant_dto_envelope_free(grant_dto_envelope_t *grant_dto_envelope) {
    ...
    if (grant_dto_envelope->error_code) {
        free(grant_dto_envelope->error_code);
        grant_dto_envelope->error_code = NULL;
    }
    if (grant_dto_envelope->validation_details) {
        list_ForEach(listEntry, grant_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(grant_dto_envelope->validation_details);
        grant_dto_envelope->validation_details = NULL;
    }
...
cJSON *grant_dto_envelope_convertToJSON(grant_dto_envelope_t *grant_dto_envelope) {
    ...
    // grant_dto_envelope->http_status
    if(grant_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", grant_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // grant_dto_envelope->error_code
    if(grant_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", grant_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // grant_dto_envelope->validation_details
    if(grant_dto_envelope->validation_details) {
    ...
    if (grant_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, grant_dto_envelope->validation_details) {
...
grant_dto_envelope_t *grant_dto_envelope_parseFromJSON(cJSON *grant_dto_envelopeJSON){
    ...
    // define the local map for grant_dto_envelope->validation_details
    ...
    // grant_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(grant_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // grant_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(grant_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // grant_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(grant_dto_envelopeJSON, "validationDetails");
