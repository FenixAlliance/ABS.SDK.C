...
transaction_category_dto_list_envelope_t *transaction_category_dto_list_envelope_create(
    ...
    transaction_category_dto_list_envelope_local_var->http_status = http_status;
    transaction_category_dto_list_envelope_local_var->error_code = error_code;
    transaction_category_dto_list_envelope_local_var->validation_details = validation_details;
...
void transaction_category_dto_list_envelope_free(transaction_category_dto_list_envelope_t *transaction_category_dto_list_envelope) {
    ...
    if (transaction_category_dto_list_envelope->error_code) {
        free(transaction_category_dto_list_envelope->error_code);
        transaction_category_dto_list_envelope->error_code = NULL;
    }
    if (transaction_category_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, transaction_category_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(transaction_category_dto_list_envelope->validation_details);
        transaction_category_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *transaction_category_dto_list_envelope_convertToJSON(transaction_category_dto_list_envelope_t *transaction_category_dto_list_envelope) {
    ...
    // transaction_category_dto_list_envelope->http_status
    if(transaction_category_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", transaction_category_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // transaction_category_dto_list_envelope->error_code
    if(transaction_category_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", transaction_category_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // transaction_category_dto_list_envelope->validation_details
    if(transaction_category_dto_list_envelope->validation_details) {
    ...
    if (transaction_category_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, transaction_category_dto_list_envelope->validation_details) {
...
transaction_category_dto_list_envelope_t *transaction_category_dto_list_envelope_parseFromJSON(cJSON *transaction_category_dto_list_envelopeJSON){
    ...
    // define the local map for transaction_category_dto_list_envelope->validation_details
    ...
    // transaction_category_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(transaction_category_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // transaction_category_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(transaction_category_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // transaction_category_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(transaction_category_dto_list_envelopeJSON, "validationDetails");
