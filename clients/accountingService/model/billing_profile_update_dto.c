...
char* billing_profile_update_dto_tax_payer_type_ToString(accountingservice_billing_profile_update_dto_TAXPAYERTYPE_e tax_payer_type) {
    char* tax_payer_typeArray[] =  { "NULL", "Individual", "Business" };
    return tax_payer_typeArray[tax_payer_type];
}

accountingservice_billing_profile_update_dto_TAXPAYERTYPE_e billing_profile_update_dto_tax_payer_type_FromString(char* tax_payer_type){
...
billing_profile_update_dto_t *billing_profile_update_dto_create(
    ...
    accountingservice_billing_profile_update_dto_TAXPAYERTYPE_e tax_payer_type,
    char *country_id,
    char *state_id,
    char *city_id,
    ...
    billing_profile_update_dto_local_var->verification_digit = verification_digit;
    billing_profile_update_dto_local_var->economic_activity_code = economic_activity_code;
    billing_profile_update_dto_local_var->economic_activity_scheme = economic_activity_scheme;
    billing_profile_update_dto_local_var->is_primary = is_primary;
    billing_profile_update_dto_local_var->establishment_code = establishment_code;
    ...
    billing_profile_update_dto_local_var->tax_payer_type = tax_payer_type;
...
void billing_profile_update_dto_free(billing_profile_update_dto_t *billing_profile_update_dto) {
    ...
    if (billing_profile_update_dto->verification_digit) {
        free(billing_profile_update_dto->verification_digit);
        billing_profile_update_dto->verification_digit = NULL;
    }
    if (billing_profile_update_dto->economic_activity_code) {
        free(billing_profile_update_dto->economic_activity_code);
        billing_profile_update_dto->economic_activity_code = NULL;
    }
    if (billing_profile_update_dto->economic_activity_scheme) {
        free(billing_profile_update_dto->economic_activity_scheme);
        billing_profile_update_dto->economic_activity_scheme = NULL;
    }
    if (billing_profile_update_dto->establishment_code) {
        free(billing_profile_update_dto->establishment_code);
        billing_profile_update_dto->establishment_code = NULL;
    }
...
cJSON *billing_profile_update_dto_convertToJSON(billing_profile_update_dto_t *billing_profile_update_dto) {
    ...
    // billing_profile_update_dto->verification_digit
    if(billing_profile_update_dto->verification_digit) {
    if(cJSON_AddStringToObject(item, "verificationDigit", billing_profile_update_dto->verification_digit) == NULL) {
    goto fail; //String
    }
    }


    // billing_profile_update_dto->economic_activity_code
    if(billing_profile_update_dto->economic_activity_code) {
    if(cJSON_AddStringToObject(item, "economicActivityCode", billing_profile_update_dto->economic_activity_code) == NULL) {
    goto fail; //String
    }
    }


    // billing_profile_update_dto->economic_activity_scheme
    if(billing_profile_update_dto->economic_activity_scheme) {
    if(cJSON_AddStringToObject(item, "economicActivityScheme", billing_profile_update_dto->economic_activity_scheme) == NULL) {
    goto fail; //String
    }
    }


    // billing_profile_update_dto->is_primary
    if(billing_profile_update_dto->is_primary) {
    if(cJSON_AddBoolToObject(item, "isPrimary", billing_profile_update_dto->is_primary) == NULL) {
    goto fail; //Bool
    }
    }


    // billing_profile_update_dto->establishment_code
    if(billing_profile_update_dto->establishment_code) {
    if(cJSON_AddStringToObject(item, "establishmentCode", billing_profile_update_dto->establishment_code) == NULL) {
    goto fail; //String
    }
    }
    ...
    // billing_profile_update_dto->tax_payer_type
    if(billing_profile_update_dto->tax_payer_type != accountingservice_billing_profile_update_dto_TAXPAYERTYPE_NULL) {
    if(cJSON_AddStringToObject(item, "taxPayerType", tax_payer_typebilling_profile_update_dto_ToString(billing_profile_update_dto->tax_payer_type)) == NULL)
    {
    goto fail; //Enum
    }
    }
...
billing_profile_update_dto_t *billing_profile_update_dto_parseFromJSON(cJSON *billing_profile_update_dtoJSON){
    ...
    // billing_profile_update_dto->verification_digit
    cJSON *verification_digit = cJSON_GetObjectItemCaseSensitive(billing_profile_update_dtoJSON, "verificationDigit");
    ...
    // billing_profile_update_dto->economic_activity_code
    cJSON *economic_activity_code = cJSON_GetObjectItemCaseSensitive(billing_profile_update_dtoJSON, "economicActivityCode");
    ...
    // billing_profile_update_dto->economic_activity_scheme
    cJSON *economic_activity_scheme = cJSON_GetObjectItemCaseSensitive(billing_profile_update_dtoJSON, "economicActivityScheme");
    ...
    // billing_profile_update_dto->is_primary
    cJSON *is_primary = cJSON_GetObjectItemCaseSensitive(billing_profile_update_dtoJSON, "isPrimary");
    if (is_primary) { 
    ...
    // billing_profile_update_dto->establishment_code
    cJSON *establishment_code = cJSON_GetObjectItemCaseSensitive(billing_profile_update_dtoJSON, "establishmentCode");
    ...
    // billing_profile_update_dto->tax_payer_type
    cJSON *tax_payer_type = cJSON_GetObjectItemCaseSensitive(billing_profile_update_dtoJSON, "taxPayerType");
    accountingservice_billing_profile_update_dto_TAXPAYERTYPE_e tax_payer_typeVariable;
    if (tax_payer_type) { 
    if(!cJSON_IsString(tax_payer_type))
    {
    goto end; //Enum
    }
    tax_payer_typeVariable = billing_profile_update_dto_tax_payer_type_FromString(tax_payer_type->valuestring);
    }
    ...
    billing_profile_update_dto_local_var = billing_profile_update_dto_create (
        ...
        tax_payer_type ? tax_payer_typeVariable : accountingservice_billing_profile_update_dto_TAXPAYERTYPE_NULL,
