...
loan_dto_envelope_t *loan_dto_envelope_create(
    ...
    loan_dto_envelope_local_var->http_status = http_status;
    loan_dto_envelope_local_var->error_code = error_code;
    loan_dto_envelope_local_var->validation_details = validation_details;
...
void loan_dto_envelope_free(loan_dto_envelope_t *loan_dto_envelope) {
    ...
    if (loan_dto_envelope->error_code) {
        free(loan_dto_envelope->error_code);
        loan_dto_envelope->error_code = NULL;
    }
    if (loan_dto_envelope->validation_details) {
        list_ForEach(listEntry, loan_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(loan_dto_envelope->validation_details);
        loan_dto_envelope->validation_details = NULL;
    }
...
cJSON *loan_dto_envelope_convertToJSON(loan_dto_envelope_t *loan_dto_envelope) {
    ...
    // loan_dto_envelope->http_status
    if(loan_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", loan_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // loan_dto_envelope->error_code
    if(loan_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", loan_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // loan_dto_envelope->validation_details
    if(loan_dto_envelope->validation_details) {
    ...
    if (loan_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, loan_dto_envelope->validation_details) {
...
loan_dto_envelope_t *loan_dto_envelope_parseFromJSON(cJSON *loan_dto_envelopeJSON){
    ...
    // define the local map for loan_dto_envelope->validation_details
    ...
    // loan_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(loan_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // loan_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(loan_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // loan_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(loan_dto_envelopeJSON, "validationDetails");
