...
transaction_dto_envelope_t *transaction_dto_envelope_create(
    ...
    transaction_dto_envelope_local_var->http_status = http_status;
    transaction_dto_envelope_local_var->error_code = error_code;
    transaction_dto_envelope_local_var->validation_details = validation_details;
...
void transaction_dto_envelope_free(transaction_dto_envelope_t *transaction_dto_envelope) {
    ...
    if (transaction_dto_envelope->error_code) {
        free(transaction_dto_envelope->error_code);
        transaction_dto_envelope->error_code = NULL;
    }
    if (transaction_dto_envelope->validation_details) {
        list_ForEach(listEntry, transaction_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(transaction_dto_envelope->validation_details);
        transaction_dto_envelope->validation_details = NULL;
    }
...
cJSON *transaction_dto_envelope_convertToJSON(transaction_dto_envelope_t *transaction_dto_envelope) {
    ...
    // transaction_dto_envelope->http_status
    if(transaction_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", transaction_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // transaction_dto_envelope->error_code
    if(transaction_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", transaction_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // transaction_dto_envelope->validation_details
    if(transaction_dto_envelope->validation_details) {
    ...
    if (transaction_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, transaction_dto_envelope->validation_details) {
...
transaction_dto_envelope_t *transaction_dto_envelope_parseFromJSON(cJSON *transaction_dto_envelopeJSON){
    ...
    // define the local map for transaction_dto_envelope->validation_details
    ...
    // transaction_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(transaction_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // transaction_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(transaction_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // transaction_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(transaction_dto_envelopeJSON, "validationDetails");
