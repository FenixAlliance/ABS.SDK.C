...
journal_type_dto_envelope_t *journal_type_dto_envelope_create(
    ...
    journal_type_dto_envelope_local_var->http_status = http_status;
    journal_type_dto_envelope_local_var->error_code = error_code;
    journal_type_dto_envelope_local_var->validation_details = validation_details;
...
void journal_type_dto_envelope_free(journal_type_dto_envelope_t *journal_type_dto_envelope) {
    ...
    if (journal_type_dto_envelope->error_code) {
        free(journal_type_dto_envelope->error_code);
        journal_type_dto_envelope->error_code = NULL;
    }
    if (journal_type_dto_envelope->validation_details) {
        list_ForEach(listEntry, journal_type_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(journal_type_dto_envelope->validation_details);
        journal_type_dto_envelope->validation_details = NULL;
    }
...
cJSON *journal_type_dto_envelope_convertToJSON(journal_type_dto_envelope_t *journal_type_dto_envelope) {
    ...
    // journal_type_dto_envelope->http_status
    if(journal_type_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", journal_type_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // journal_type_dto_envelope->error_code
    if(journal_type_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", journal_type_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // journal_type_dto_envelope->validation_details
    if(journal_type_dto_envelope->validation_details) {
    ...
    if (journal_type_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, journal_type_dto_envelope->validation_details) {
...
journal_type_dto_envelope_t *journal_type_dto_envelope_parseFromJSON(cJSON *journal_type_dto_envelopeJSON){
    ...
    // define the local map for journal_type_dto_envelope->validation_details
    ...
    // journal_type_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(journal_type_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // journal_type_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(journal_type_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // journal_type_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(journal_type_dto_envelopeJSON, "validationDetails");
