...
cost_centre_budget_dto_envelope_t *cost_centre_budget_dto_envelope_create(
    ...
    cost_centre_budget_dto_envelope_local_var->http_status = http_status;
    cost_centre_budget_dto_envelope_local_var->error_code = error_code;
    cost_centre_budget_dto_envelope_local_var->validation_details = validation_details;
...
void cost_centre_budget_dto_envelope_free(cost_centre_budget_dto_envelope_t *cost_centre_budget_dto_envelope) {
    ...
    if (cost_centre_budget_dto_envelope->error_code) {
        free(cost_centre_budget_dto_envelope->error_code);
        cost_centre_budget_dto_envelope->error_code = NULL;
    }
    if (cost_centre_budget_dto_envelope->validation_details) {
        list_ForEach(listEntry, cost_centre_budget_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(cost_centre_budget_dto_envelope->validation_details);
        cost_centre_budget_dto_envelope->validation_details = NULL;
    }
...
cJSON *cost_centre_budget_dto_envelope_convertToJSON(cost_centre_budget_dto_envelope_t *cost_centre_budget_dto_envelope) {
    ...
    // cost_centre_budget_dto_envelope->http_status
    if(cost_centre_budget_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", cost_centre_budget_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // cost_centre_budget_dto_envelope->error_code
    if(cost_centre_budget_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", cost_centre_budget_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // cost_centre_budget_dto_envelope->validation_details
    if(cost_centre_budget_dto_envelope->validation_details) {
    ...
    if (cost_centre_budget_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, cost_centre_budget_dto_envelope->validation_details) {
...
cost_centre_budget_dto_envelope_t *cost_centre_budget_dto_envelope_parseFromJSON(cJSON *cost_centre_budget_dto_envelopeJSON){
    ...
    // define the local map for cost_centre_budget_dto_envelope->validation_details
    ...
    // cost_centre_budget_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(cost_centre_budget_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // cost_centre_budget_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(cost_centre_budget_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // cost_centre_budget_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(cost_centre_budget_dto_envelopeJSON, "validationDetails");
