...
account_group_dto_envelope_t *account_group_dto_envelope_create(
    ...
    account_group_dto_envelope_local_var->http_status = http_status;
    account_group_dto_envelope_local_var->error_code = error_code;
    account_group_dto_envelope_local_var->validation_details = validation_details;
...
void account_group_dto_envelope_free(account_group_dto_envelope_t *account_group_dto_envelope) {
    ...
    if (account_group_dto_envelope->error_code) {
        free(account_group_dto_envelope->error_code);
        account_group_dto_envelope->error_code = NULL;
    }
    if (account_group_dto_envelope->validation_details) {
        list_ForEach(listEntry, account_group_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(account_group_dto_envelope->validation_details);
        account_group_dto_envelope->validation_details = NULL;
    }
...
cJSON *account_group_dto_envelope_convertToJSON(account_group_dto_envelope_t *account_group_dto_envelope) {
    ...
    // account_group_dto_envelope->http_status
    if(account_group_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", account_group_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // account_group_dto_envelope->error_code
    if(account_group_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", account_group_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // account_group_dto_envelope->validation_details
    if(account_group_dto_envelope->validation_details) {
    ...
    if (account_group_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, account_group_dto_envelope->validation_details) {
...
account_group_dto_envelope_t *account_group_dto_envelope_parseFromJSON(cJSON *account_group_dto_envelopeJSON){
    ...
    // define the local map for account_group_dto_envelope->validation_details
    ...
    // account_group_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(account_group_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // account_group_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(account_group_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // account_group_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(account_group_dto_envelopeJSON, "validationDetails");
