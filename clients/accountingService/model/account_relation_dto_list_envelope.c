...
account_relation_dto_list_envelope_t *account_relation_dto_list_envelope_create(
    ...
    account_relation_dto_list_envelope_local_var->http_status = http_status;
    account_relation_dto_list_envelope_local_var->error_code = error_code;
    account_relation_dto_list_envelope_local_var->validation_details = validation_details;
...
void account_relation_dto_list_envelope_free(account_relation_dto_list_envelope_t *account_relation_dto_list_envelope) {
    ...
    if (account_relation_dto_list_envelope->error_code) {
        free(account_relation_dto_list_envelope->error_code);
        account_relation_dto_list_envelope->error_code = NULL;
    }
    if (account_relation_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, account_relation_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(account_relation_dto_list_envelope->validation_details);
        account_relation_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *account_relation_dto_list_envelope_convertToJSON(account_relation_dto_list_envelope_t *account_relation_dto_list_envelope) {
    ...
    // account_relation_dto_list_envelope->http_status
    if(account_relation_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", account_relation_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // account_relation_dto_list_envelope->error_code
    if(account_relation_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", account_relation_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // account_relation_dto_list_envelope->validation_details
    if(account_relation_dto_list_envelope->validation_details) {
    ...
    if (account_relation_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, account_relation_dto_list_envelope->validation_details) {
...
account_relation_dto_list_envelope_t *account_relation_dto_list_envelope_parseFromJSON(cJSON *account_relation_dto_list_envelopeJSON){
    ...
    // define the local map for account_relation_dto_list_envelope->validation_details
    ...
    // account_relation_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(account_relation_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // account_relation_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(account_relation_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // account_relation_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(account_relation_dto_list_envelopeJSON, "validationDetails");
