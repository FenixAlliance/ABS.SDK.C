...
discount_dto_list_envelope_t *discount_dto_list_envelope_create(
    ...
    discount_dto_list_envelope_local_var->http_status = http_status;
    discount_dto_list_envelope_local_var->error_code = error_code;
    discount_dto_list_envelope_local_var->validation_details = validation_details;
...
void discount_dto_list_envelope_free(discount_dto_list_envelope_t *discount_dto_list_envelope) {
    ...
    if (discount_dto_list_envelope->error_code) {
        free(discount_dto_list_envelope->error_code);
        discount_dto_list_envelope->error_code = NULL;
    }
    if (discount_dto_list_envelope->validation_details) {
        list_ForEach(listEntry, discount_dto_list_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(discount_dto_list_envelope->validation_details);
        discount_dto_list_envelope->validation_details = NULL;
    }
...
cJSON *discount_dto_list_envelope_convertToJSON(discount_dto_list_envelope_t *discount_dto_list_envelope) {
    ...
    // discount_dto_list_envelope->http_status
    if(discount_dto_list_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", discount_dto_list_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // discount_dto_list_envelope->error_code
    if(discount_dto_list_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", discount_dto_list_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // discount_dto_list_envelope->validation_details
    if(discount_dto_list_envelope->validation_details) {
    ...
    if (discount_dto_list_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, discount_dto_list_envelope->validation_details) {
...
discount_dto_list_envelope_t *discount_dto_list_envelope_parseFromJSON(cJSON *discount_dto_list_envelopeJSON){
    ...
    // define the local map for discount_dto_list_envelope->validation_details
    ...
    // discount_dto_list_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(discount_dto_list_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // discount_dto_list_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(discount_dto_list_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // discount_dto_list_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(discount_dto_list_envelopeJSON, "validationDetails");
