...
discount_list_dto_envelope_t *discount_list_dto_envelope_create(
    ...
    discount_list_dto_envelope_local_var->http_status = http_status;
    discount_list_dto_envelope_local_var->error_code = error_code;
    discount_list_dto_envelope_local_var->validation_details = validation_details;
...
void discount_list_dto_envelope_free(discount_list_dto_envelope_t *discount_list_dto_envelope) {
    ...
    if (discount_list_dto_envelope->error_code) {
        free(discount_list_dto_envelope->error_code);
        discount_list_dto_envelope->error_code = NULL;
    }
    if (discount_list_dto_envelope->validation_details) {
        list_ForEach(listEntry, discount_list_dto_envelope->validation_details) {
            keyValuePair_t *localKeyValue = (keyValuePair_t*) listEntry->data;
            free (localKeyValue->key);
            free (localKeyValue->value);
            keyValuePair_free(localKeyValue);
        }
        list_freeList(discount_list_dto_envelope->validation_details);
        discount_list_dto_envelope->validation_details = NULL;
    }
...
cJSON *discount_list_dto_envelope_convertToJSON(discount_list_dto_envelope_t *discount_list_dto_envelope) {
    ...
    // discount_list_dto_envelope->http_status
    if(discount_list_dto_envelope->http_status) {
    if(cJSON_AddNumberToObject(item, "httpStatus", discount_list_dto_envelope->http_status) == NULL) {
    goto fail; //Numeric
    }
    }


    // discount_list_dto_envelope->error_code
    if(discount_list_dto_envelope->error_code) {
    if(cJSON_AddStringToObject(item, "errorCode", discount_list_dto_envelope->error_code) == NULL) {
    goto fail; //String
    }
    }


    // discount_list_dto_envelope->validation_details
    if(discount_list_dto_envelope->validation_details) {
    ...
    if (discount_list_dto_envelope->validation_details) {
    list_ForEach(validation_detailsListEntry, discount_list_dto_envelope->validation_details) {
...
discount_list_dto_envelope_t *discount_list_dto_envelope_parseFromJSON(cJSON *discount_list_dto_envelopeJSON){
    ...
    // define the local map for discount_list_dto_envelope->validation_details
    ...
    // discount_list_dto_envelope->http_status
    cJSON *http_status = cJSON_GetObjectItemCaseSensitive(discount_list_dto_envelopeJSON, "httpStatus");
    if (http_status) { 
    ...
    // discount_list_dto_envelope->error_code
    cJSON *error_code = cJSON_GetObjectItemCaseSensitive(discount_list_dto_envelopeJSON, "errorCode");
    if (error_code) { 
    ...
    // discount_list_dto_envelope->validation_details
    cJSON *validation_details = cJSON_GetObjectItemCaseSensitive(discount_list_dto_envelopeJSON, "validationDetails");
